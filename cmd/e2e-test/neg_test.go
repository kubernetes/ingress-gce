...
func TestNEGBasic(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		Framework.RunWithSandbox(tc.desc, t, func(t *testing.T, s *e2e.Sandbox) {
			...
			negEnable := annotations.NegAnnotation{Ingress: true}
			annotation := map[string]string{}
			annotation[annotations.NEGAnnotationKey] = negEnable.String()
			_, _, err := e2e.CreateEchoService(s, "service-1", annotation)
			if err != nil {
