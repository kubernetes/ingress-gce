...
func runL4Controllers(ctx *ingctx.ControllerContext, systemHealth *systemhealth.SystemHealth, option runOption, leOption leaderElectionOption, logger klog.Logger) {
	...
	consistencyStore := newConsistencyStore(ctx, logger)

	if flags.F.RunL4Controller {
		l4Controller := controllers.NewILBController(ctx, option.stopCh, logger, consistencyStore)
	...
	if flags.F.RunL4NetLBController {
		l4netlbController := controllers.NewL4NetLBController(ctx, option.stopCh, logger, consistencyStore)
	...
	if flags.F.RunL4StandaloneNEGController {
		standaloneNEGLBController := controllers.NewStandaloneNEGLBController(ctx, option.stopCh, logger, consistencyStore)
...
// newConsistencyStore returns the ConsistencyStore shared by the L4
...
func newConsistencyStore(ctx *ingctx.ControllerContext, logger klog.Logger) consistency.ConsistencyStore {
