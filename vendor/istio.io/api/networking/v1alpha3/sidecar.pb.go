...
// ```
//
// A host can be excluded by prefixing the namespace with `~`. Excluded hosts are
// subtracted from the imported hosts, letting a sidecar import broadly and then
// carve out a few namespaces or services. The example below imports every service
// in the mesh except those in the `prod-us2` namespace, the `foo.example.com`
// service in `prod-apis`, and the `bar.example.com` service in every namespace.
//
// ```yaml
// apiVersion: networking.istio.io/v1
// kind: Sidecar
...
type IstioEgressListener struct {
	...
	//
	// A host may also be excluded by prefixing the `namespace` with `~`. An
	// exclusion entry follows the exact same `namespace/dnsName` convention as an
	// inclusion entry, so `~prod-us2/*` excludes every host in the `prod-us2`
	// namespace. Note that a bare `~` namespace is treated as the wildcard
	// namespace `*`, so `~/dnsName` (and `~/*`) selects across all namespaces.
	// Excluded hosts are subtracted from the set of imported hosts, so a host is
	// exposed only when it is imported by some entry and not excluded by any entry.
	//
