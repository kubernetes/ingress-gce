...
// List lists all GKENetworkParamSets in the indexer.
func (s *gKENetworkParamSetLister) List(selector labels.Selector) (ret []*v1.GKENetworkParamSet, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.GKENetworkParamSet))
	})
	return ret, err
}

// Get retrieves the GKENetworkParamSet from the index for a given name.
func (s *gKENetworkParamSetLister) Get(name string) (*v1.GKENetworkParamSet, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("gkenetworkparamset"), name)
	}
	return obj.(*v1.GKENetworkParamSet), nil
}
