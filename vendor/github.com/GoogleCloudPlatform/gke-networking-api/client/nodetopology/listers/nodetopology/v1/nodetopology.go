...
func NewNodeTopologyLister(indexer cache.Indexer) NodeTopologyLister {
	return &nodeTopologyLister{indexer: indexer}
}

// List lists all NodeTopologies in the indexer.
func (s *nodeTopologyLister) List(selector labels.Selector) (ret []*v1.NodeTopology, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.NodeTopology))
	})
	return ret, err
}

// Get retrieves the NodeTopology from the index for a given name.
func (s *nodeTopologyLister) Get(name string) (*v1.NodeTopology, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("nodetopology"), name)
	}
	return obj.(*v1.NodeTopology), nil
}
