...
func TestExportsCorrectDenyMetrics(t *testing.T) {
	...
	testCases := []struct {
		...
		denyEnabled bool
		fwErrors    bool
		want        metrics.FirewallIPFamilies
	}{
		{
			desc:        "ipv4",
			ipFamilies:  []v1.IPFamily{v1.IPv4Protocol},
			denyEnabled: true,
			want:        metrics.FirewallIPFamiliesIPv4,
		},
		{
			desc:        "ipv6",
			ipFamilies:  []v1.IPFamily{v1.IPv6Protocol},
			denyEnabled: true,
			want:        metrics.FirewallIPFamiliesIPv6,
		},
		{
			desc:        "ipv4_ipv6",
			ipFamilies:  []v1.IPFamily{v1.IPv4Protocol, v1.IPv6Protocol},
			denyEnabled: true,
			want:        metrics.FirewallIPFamiliesDualStack,
		},
		{
			desc:        "ipv4_disabled",
			ipFamilies:  []v1.IPFamily{v1.IPv4Protocol},
			denyEnabled: false,
			want:        metrics.FirewallIPFamiliesNone,
		},
		{
			desc:        "ipv6_disabled",
			ipFamilies:  []v1.IPFamily{v1.IPv6Protocol},
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			got := res.MetricsState.DenyFirewallFamilies
