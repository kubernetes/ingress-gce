...
func TestEnsureInternalLoadBalancer_L4LBConfigLogging(t *testing.T) {
	...
	loggingEnabled := &composite.BackendServiceLogConfig{Enable: true, SampleRate: 1.0}
	...
	enabledConfig := &l4lbconfigv1.LoggingConfig{Enabled: true, SampleRate: ptrToInt32(1000000)}
	disabledConfig := &l4lbconfigv1.LoggingConfig{Enabled: false}
	complexConfig := &l4lbconfigv1.LoggingConfig{
		Enabled:        true,
		SampleRate:     ptrToInt32(500000), // 0.5
		OptionalMode:   "INCLUDE_ALL_OPTIONAL",
		OptionalFields: []string{"field1", "field2"},
	}
	...
	testCases := []struct {
		...
		{
			desc:              "Flag ON, CRD field omitted: Preserve LKG state",
			manageLoggingFlag: true,
			existingGCEConfig: loggingEnabled,
		...
		{
			desc:              "CRD Object Deleted: Preserve LKG state and issue warning",
			manageLoggingFlag: true,
			existingGCEConfig: loggingEnabled,
	...
	for _, tc := range testCases {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.existingGCEConfig != nil {
				...
				composite.CreateBackendService(fakeGCE, key, existingBS, klog.TODO())
			}
...
// Helper to handle pointer types in test cases
