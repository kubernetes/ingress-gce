...
// List lists all ServiceAttachments in the indexer.
func (s *serviceAttachmentLister) List(selector labels.Selector) (ret []*v1alpha1.ServiceAttachment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ServiceAttachment))
	})
	return ret, err
}
...
// List lists all ServiceAttachments in the indexer for a given namespace.
func (s serviceAttachmentNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ServiceAttachment, err error) {
...
// Get retrieves the ServiceAttachment from the indexer for a given namespace and name.
func (s serviceAttachmentNamespaceLister) Get(name string) (*v1alpha1.ServiceAttachment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("serviceattachment"), name)
	}
	return obj.(*v1alpha1.ServiceAttachment), nil
}
