...
// serviceRef returns the ConsistencyStore reference for a service.
func serviceRef(namespace, name string) consistency.ObjectRef {
	return consistency.ObjectRef{
		Resource:       ServicesGroupResource,
...
func recordServiceWrite(store consistency.ConsistencyStore, uid types.UID, writtenSvc *v1.Service) {
	store.WroteAt(serviceRef(writtenSvc.Namespace, writtenSvc.Name), uid, writtenSvc.ResourceVersion)
}
