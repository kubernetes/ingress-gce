...
func NewILBController(ctx *context.ControllerContext, stopCh <-chan struct{}, logger klog.Logger, consistencyStore consistency.ConsistencyStore) *L4Controller {
	...
	ctx.ServiceInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		// Deletes are handled in the Update when the deletion timestamp is
		// set; here only drop the ConsistencyStore record so a deleted
		// service does not leak an entry or stall a future recreation.
		DeleteFunc: func(obj interface{}) {
			if svc := serviceFromDeleteEvent(obj); svc != nil {
				l4c.consistencyStore.Clear(types.NamespacedName{Namespace: svc.Namespace, Name: svc.Name}, svc.UID)
			}
		},
	})
...
func (l4c *L4Controller) processServiceCreateOrUpdate(service *v1.Service, svcLogger klog.Logger) *resources.L4ILBSyncResult {
	...
	// Ensure v2 finalizer
	service, err := l4c.ensureServiceFinalizer(service, common.ILBFinalizerV2, svcLogger)
	if err != nil {
		return &resources.L4ILBSyncResult{Error: fmt.Errorf("Failed to attach finalizer to service %s/%s, err %v", service.Namespace, service.Name, err)}
	}
	nodes, err := l4c.zoneGetter.ListNodes(zonegetter.CandidateNodesFilter, svcLogger)
	if err != nil {
		return &resources.L4ILBSyncResult{Error: err}
	}
	...
	l4 := resources.NewL4Handler(l4ilbParams, svcLogger)
...
func (l4c *L4Controller) processServiceDeletion(key string, svc *v1.Service, svcLogger klog.Logger) *resources.L4ILBSyncResult {
	startTime := time.Now()
	...
	svc, err = l4c.ensureDeleteServiceFinalizer(svc, common.ILBFinalizerV2, svcLogger)
	if err != nil {
...
func (l4c *L4Controller) linkNEG(l4 *resources.L4, svcLogger klog.Logger) error {
...
func (l4c *L4Controller) sync(key string, svcLogger klog.Logger) error {
	...
	if namespace, name, keyErr := cache.SplitMetaNamespaceKey(key); keyErr == nil {
		...
		if err := l4c.consistencyStore.EnsureReady(namespacedName); err != nil {
	...
	if !exists || svc == nil {
		...
		if namespace, name, keyErr := cache.SplitMetaNamespaceKey(key); keyErr == nil {
			l4c.consistencyStore.Clear(types.NamespacedName{Namespace: namespace, Name: name}, "")
		}
	...
	var result *resources.L4ILBSyncResult
...
func (l4c *L4Controller) publishMetrics(result *resources.L4ILBSyncResult, namespacedName string, isResync bool, svcLogger klog.Logger) {
	if result == nil {
		return
	}
...
// The wrappers below delegate service mutations to the shared helpers and
// record the resulting ResourceVersion in the ConsistencyStore, so the next
// sync of the service can be stalled until the informer cache has observed
// the write. Each returns the freshest known version of the service (the
// passed-in service if nothing was written); the result is never nil.
...
func (l4c *L4Controller) updateServiceStatus(svc *v1.Service, newStatus *v1.LoadBalancerStatus, newConditions []metav1.Condition, conditionsToRemove []string, svcLogger klog.Logger) (*v1.Service, error) {
	...
	if err == nil {
		recordServiceWrite(l4c.consistencyStore, svc.UID, newSvc)
	}
	return newSvc, err
}
...
func (l4c *L4Controller) ensureServiceFinalizer(svc *v1.Service, key string, svcLogger klog.Logger) (*v1.Service, error) {
	newSvc, err := common.EnsureServiceFinalizer(svc, key, l4c.ctx.KubeClient, svcLogger)
	if err == nil {
		recordServiceWrite(l4c.consistencyStore, svc.UID, newSvc)
	}
	return newSvc, err
...
func (l4c *L4Controller) ensureDeleteServiceFinalizer(svc *v1.Service, key string, svcLogger klog.Logger) (*v1.Service, error) {
	newSvc, err := common.EnsureDeleteServiceFinalizer(svc, key, l4c.ctx.KubeClient, svcLogger)
	if err == nil {
		recordServiceWrite(l4c.consistencyStore, svc.UID, newSvc)
	}
	return newSvc, err
