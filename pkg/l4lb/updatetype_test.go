...
func TestIsResync(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.desc, func(t *testing.T) {
			...
			versionsTracker.SetLastUpdateSeen(svcKey, tc.lastUpdate)
			versionsTracker.SetLastIgnored(svcKey, tc.lastIgnored)
			versionsTracker.SetProcessed(svcKey, tc.lastProcessedVersion, tc.lastProcessedSuccess, false)
...
func TestSetProcessedDoesNotChangeLastProcessedUpdateIfOperationWasResync(t *testing.T) {
	...
	versionsTracker.SetProcessed(svcKey, "2", true, true)
...
func TestSetProcessedChangesLastProcessedUpdateIfOperationWasUpdate(t *testing.T) {
	...
	versionsTracker.SetProcessed(svcKey, "2", true, false)
