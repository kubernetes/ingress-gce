...
func (t *serviceVersionsTracker) SetLastUpdateSeen(svcKey, version string) {
	t.lock.Lock()
	defer t.lock.Unlock()
...
func (t *serviceVersionsTracker) SetLastIgnored(svcKey, version string) {
	t.lock.Lock()
	defer t.lock.Unlock()
...
func (t *serviceVersionsTracker) SetProcessed(svcKey, version string, success, wasResync bool) {
...
func (t *serviceVersionsTracker) IsResync(svcKey, currentVersion string, logger klog.Logger) bool {
	...
	logger.V(3).Info("IsResync()", "currentVersion", currentVersion, "lastProcessedUpdateVersion", v.lastProcessedUpdateVersion, "lastProcessingSuccess", v.lastProcessingSuccess, "lastSeenUpdate", v.lastSeenUpdateVersion, "lastIgnored", v.lastIgnoredVersion)
