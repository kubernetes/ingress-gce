...
func (lc *L4NetLBController) sync(key string, svcLogger klog.Logger) error {
	...
	if lc.ctx.ReadOnlyMode {
		svcLogger.Info("Skipping syncing L4 NetLB RBS service since the controller is in read-only mode", "service", svc)
		return nil
	}
...
func (lc *L4NetLBController) syncInternal(service *v1.Service, svcLogger klog.Logger) *loadbalancers.L4NetLBSyncResult {
	...
	if lc.ctx.ReadOnlyMode {
		svcLogger.Info("Skipping syncing L4 NetLB RBS service since the controller is in read-only mode")
		return nil
	}
