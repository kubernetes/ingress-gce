...
func TestProcessCreateOrUpdate(t *testing.T) {
	...
	prevMetrics := test.GetL4LatencyMetric(t, l4ilbLatencyMetricName)
	...
	prevMetrics.ValidateDiff(test.GetL4LatencyMetric(t, l4ilbLatencyMetricName), &test.L4LBLatencyMetricInfo{CreateCount: 1, UpperBoundSeconds: 1}, t)
	...
	validateSvcStatus(newSvc, true, t)
	prevMetrics.ValidateDiff(test.GetL4LatencyMetric(t, l4ilbLatencyMetricName), &test.L4LBLatencyMetricInfo{CreateCount: 1, UpdateCount: 1, UpperBoundSeconds: 1}, t)
	...
	prevMetrics.ValidateDiff(test.GetL4LatencyMetric(t, l4ilbLatencyMetricName), &test.L4LBLatencyMetricInfo{CreateCount: 1, UpdateCount: 1, DeleteCount: 1, UpperBoundSeconds: 1}, t)
...
func TestProcessDeletion(t *testing.T) {
	...
	validateSvcStatus(newSvc, false, t)
	prevMetrics.ValidateDiff(test.GetL4LatencyMetric(t, l4ilbLatencyMetricName), &test.L4LBLatencyMetricInfo{CreateCount: 1, DeleteCount: 1, UpperBoundSeconds: 1}, t)
...
func TestProcessCreateLegacyService(t *testing.T) {
	...
	prevMetrics.ValidateDiff(test.GetL4LatencyMetric(t, l4ilbLatencyMetricName), &test.L4LBLatencyMetricInfo{}, t)
}
...
func TestProcessMultipleServices(t *testing.T) {
	...
	for _, onlyLocal := range []bool{true, false} {
		t.Run(fmt.Sprintf("L4 with LocalMode=%v", onlyLocal), func(t *testing.T) {
			...
			// this will be a create metric since an ILB IP is being assigned for the first time.
			prevMetrics.ValidateDiff(test.GetL4LatencyMetric(t, l4ilbLatencyMetricName), &test.L4LBLatencyMetricInfo{CreateCount: 20, UpperBoundSeconds: 1}, t)
...
func TestProcessServiceOnError(t *testing.T) {
	...
	prevMetrics := test.GetL4LBErrorMetric(t, l4ilbErrorMetricName)
	newSvc := test.NewL4ILBService(false, 8080)
	addILBService(l4c, newSvc)
	addNEG(l4c, newSvc)
	err := l4c.sync(getKeyForSvc(newSvc, t))
	...
	prevMetrics.ValidateDiff(test.GetL4LBErrorMetric(t, l4ilbErrorMetricName), expectMetrics, t)
}
