...
func (m *manager) EnsureInstanceGroupsAndPorts(name string, ports []int64, logger klog.Logger) (igs []*compute.InstanceGroup, err error) {
	...
	if m.readOnlyMode {
		iglogger.Info("Skipping ensuring instance group and port since ig manager is in read-only mode")
...
func (m *manager) Sync(nodes []string, logger klog.Logger) (err error) {
	...
	if m.readOnlyMode {
		iglogger.Info("Skipping syncing node since controller is in read-only mode")
		return nil
	}
