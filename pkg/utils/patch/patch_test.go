...
func TestPatchClusterSliceObjectMetadata(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			csClient := clusterslicefake.NewSimpleClientset()
			if _, err := csClient.FlagsV1().ClusterSlices(tc.cs.Namespace).Create(context.TODO(), tc.cs, metav1.CreateOptions{}); err != nil {
			...
			gotCS, err := csClient.FlagsV1().ClusterSlices(tc.cs.Namespace).Get(context.TODO(), tc.cs.Name, metav1.GetOptions{})
			if err != nil {
