...
func PatchProviderConfigObjectMetadata(client providerconfigclient.Interface, pc *providerconfig.ProviderConfig, newObjectMetadata metav1.ObjectMeta) error {
	...
	_, err = client.ProviderconfigV1().ProviderConfigs(newPC.Namespace).Patch(context.Background(), newPC.Name, types.StrategicMergePatchType, patchBytes, metav1.PatchOptions{})
	return err
}
