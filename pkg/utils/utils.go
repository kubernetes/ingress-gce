...
func EqualForwardingRules(fr1, fr2 *composite.ForwardingRule) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("EqualForwardingRules(): failed to parse backend resource URL from FR, err - %w", err)
	}
	id2, err := cloud.ParseResourceURL(fr2.BackendService)
	if err != nil {
		return false, fmt.Errorf("EqualForwardingRules(): failed to parse resource URL from FR, err - %w", err)
	}
