...
type Namer struct {
	...
	nameLock sync.Mutex
	// principalEntityName could be cluster name, or tenant UID in multi-project scenario
...
func NewNamerWithOptions(prefix, principalEntityName, firewallName string, maxNEGLabelLength int, schemaVersion string, logger klog.Logger) *Namer {
	namer := &Namer{
		prefix:            prefix,
		logger:            logger.WithName("Namer"),
