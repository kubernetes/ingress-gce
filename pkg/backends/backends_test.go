...
func TestHigherPrecedenceAPIVersion(t *testing.T) {
	testCases := []struct {
		desc     string
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			t.Parallel()
			result := higherPrecedenceAPIVersion(tc.versionA, tc.versionB)
			if result != tc.want {
				t.Errorf("higherPrecedenceAPIVersion(%s, %s) = %s, want %s", tc.versionA, tc.versionB, result, tc.want)
			}

			// Test symmetry - order of arguments shouldn't matter
			reverseResult := higherPrecedenceAPIVersion(tc.versionB, tc.versionA)
			if reverseResult != result {
				t.Errorf("higherPrecedenceAPIVersion not symmetric: higherPrecedenceAPIVersion(%s, %s) = %s, but higherPrecedenceAPIVersion(%s, %s) = %s",
