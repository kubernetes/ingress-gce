...
// GetPreprovisioningZones validates the raw pre-provisioned zones from the Service annotation
...
func GetPreprovisioningZones(service *apiv1.Service, cloud NetworkEndpointGroupCloud) ([]string, error) {

	if service == nil {
		return nil, nil
	}
	negAnnotation, found, err := negannotation.FromService(service).NEGAnnotation()
	if err != nil || !found || negAnnotation == nil {
		return nil, nil
	}
	preprovisioningZones := negAnnotation.Zones
	...
	region := cloud.Region()
	regionZones, err := cloud.Zones()
	if err != nil {
		return nil, fmt.Errorf("failed to list zones in region %q: %w", region, err)
	}
	...
	for _, zone := range preprovisioningZones {
		if len(regionZonesSet) > 0 && !regionZonesSet.Has(zone) {
			return nil, fmt.Errorf("zone %q is not in the cluster's region zones: %v", zone, regionZones)
		}
	}
