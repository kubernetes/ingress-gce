...
func (l *L7EndpointsCalculator) CalculateEndpoints(eds []types.EndpointsData, _ map[string]types.NetworkEndpointSet, inErrorState bool) (map[string]types.NetworkEndpointSet, types.EndpointPodMap, int, error) {
	if !l.enableDegradedMode {
		result := toZoneNetworkEndpointMap(eds, l.podLister, l.zoneGetter, l.servicePortName, l.networkEndpointType, l.lpConfig)
		return result.NetworkEndpointSet, result.EndpointPodMap, result.DupCount, result.Err
	}
	degradedModeResult := toZoneNetworkEndpointMapDegradedMode(eds, l.zoneGetter, l.podLister, l.nodeLister, l.servicePortName, l.networkEndpointType)
	legacyResult := toZoneNetworkEndpointMap(eds, l.podLister, l.zoneGetter, l.servicePortName, l.networkEndpointType, l.lpConfig)

	if inErrorState {
		return degradedModeResult.NetworkEndpointSet, degradedModeResult.EndpointPodMap, degradedModeResult.DupCount, degradedModeResult.Err
	}
	return legacyResult.NetworkEndpointSet, legacyResult.EndpointPodMap, legacyResult.DupCount, legacyResult.Err
}
