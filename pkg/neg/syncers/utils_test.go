...
func TestRetrieveExistingZoneNetworkEndpointMap(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		endpointSets, annotationMap, err := retrieveExistingZoneNetworkEndpointMap(tc.subnetToNegMapping, zoneGetter, negCloud, meta.VersionGA, tc.mode, false, klog.TODO(), metrics.NewNegMetrics(), false)
...
func TestRetrieveExistingZoneNetworkEndpointMapHealth(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			endpointSets, _, err := retrieveExistingZoneNetworkEndpointMap(subnetToNegMapping, zoneGetter, fakeCloud, meta.VersionGA, negtypes.L4LocalMode, false, klog.TODO(), metrics.NewNegMetrics(), tc.useHealthStatus)
			if err != nil {
