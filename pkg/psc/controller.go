...
func GetForwardingRuleStackMap(svc *v1.Service) (ForwardingRuleStackMap, error) {
	...
	if _, ok = svc.Annotations[annotations.L3ForwardingRuleKey]; ok {
		return frNames, fmt.Errorf("Service %s manages L3 ForwardingRule and cannot be used as PSC backend", svc.Name)
	}
...
func (c *Controller) processServiceAttachment(key string) error {
	...
	defer func() {
		...
		c.collector.SetServiceAttachment(key, metrics.PSCState{InSuccess: err == nil, WithUnsyncedFields: unsyncedFieldsVal != nil && *unsyncedFieldsVal != ""})
	}()
	...
	svc0bj, exists, err := c.serviceLister.GetByKey(svcKey)
	if err != nil {
...
func (c *Controller) getSubnets(subnets []string) ([]*ga.Subnetwork, error) {
	...
	for _, subnetName := range subnets {
		...
		if err == nil {
			...
			key = meta.RegionalKey(subnetName, region)
		}
		subnet, err := c.cloud.Compute().Subnetworks().Get(context2.Background(), key)
		if err != nil {
...
func (c *Controller) updateServiceAttachmentStatus(cr *sav1.ServiceAttachment, gceSAKey *meta.Key, unsyncedFieldsVal *string) (*sav1.ServiceAttachment, error) {
	...
	var consumers []sav1.ConsumerForwardingRule
	for _, c := range gceSA.ConnectedEndpoints {
		consumers = append(consumers, sav1.ConsumerForwardingRule{
	...
	if unsyncedFieldsVal != nil {
		...
		// cleanup if --enable-psc-reconcile-connections flag is disabled, otherwise when enabled again,
		// it will not be possible to define initial sync to make first check for unsynced fields
...
func (c *Controller) patchServiceAttachment(originalSA, updatedSA *sav1.ServiceAttachment) (*sav1.ServiceAttachment, error) {
	patchBytes, err := patch.MergePatchBytes(originalSA, updatedSA)
	if err != nil {
		return originalSA, err
	}
	return c.saClient.NetworkingV1().ServiceAttachments(originalSA.Namespace).Patch(context2.Background(), updatedSA.Name, types.MergePatchType, patchBytes, metav1.PatchOptions{})
}
...
func (c *Controller) ensureSAFinalizer(saCR *sav1.ServiceAttachment) (*sav1.ServiceAttachment, error) {
