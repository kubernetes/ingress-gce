...
func TestValidateHealthCheckSourceCIDRs(t *testing.T) {
	testCases := []struct {
		name      string
		input     string
		wantError bool
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			err := ValidateHealthCheckSourceCIDRs(tc.input)
			if (err != nil) != tc.wantError {
				t.Errorf("ValidateHealthCheckSourceCIDRs(%q) expected error=%v, got error=%v", tc.input, tc.wantError, err != nil)
			}
		})
	}
}
