...
// fakeProviderConfigControllersManager implements controllerManager
...
func newFakeProviderConfigControllersManager(pcClient *fakeproviderconfigclient.Clientset, finalizerName string, logger klog.Logger) *fakeProviderConfigControllersManager {
...
func (f *fakeProviderConfigControllersManager) StopControllersForProviderConfig(pc *providerconfigv1.ProviderConfig) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.stopErr != nil {
		return f.stopErr
