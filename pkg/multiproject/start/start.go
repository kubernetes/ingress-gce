	...
	return &leaderelection.LeaderElectionConfig{
		...
		Callbacks: leaderelection.LeaderCallbacks{
			OnStartedLeading: func(ctx context.Context) {
				termStop := make(chan struct{})
				go Start(logger, kubeClient, svcNegClient, kubeSystemUID, eventRecorderKubeClient, providerConfigClient, informersFactory, svcNegFactory, networkFactory, nodeTopologyFactory, gceCreator, rootNamer, termStop)
				<-ctx.Done()
				close(termStop)
				logger.Info("leadership lost, controllers stopped")
			},
