...
type AWSV4Signature struct {
	...
	// updating or
	// creating a backend that uses AWS v4 signature authentication, but
	// will not
...
type Address struct {
	...
	// The type of address to reserve, either INTERNAL orEXTERNAL. If
	// unspecified, defaults to EXTERNAL.
	AddressType string `json:"addressType,omitempty"`
	// Output only. [Output Only] Creation timestamp inRFC3339
	// text format.
	CreationTimestamp string `json:"creationTimestamp,omitempty"`
	// An optional description of this resource. Provide this field when
	// you
	// create the resource.
	Description string `json:"description,omitempty"`
	// Output only. [Output Only] The unique identifier for the resource.
	// This identifier is
	...
	// A fingerprint for the labels being applied to this Address, which
	// is
	...
	// an
	// up-to-date fingerprint hash in order to update or change
	// labels,
	...
	// global external IP addresses are always Premium Tier; regional
	// external IP
	...
	//      instances, alias IP ranges, load balancers, and similar
	// resources.
	//      - DNS_RESOLVER for a DNS resolver address in a subnetwork
	//        for a Cloud DNS  inbound
	//        forwarder IP addresses (regional internal IP address in a
	// subnet of
	//        a VPC network)
	...
	// parameter in
	// the HTTP request URL. *This field is not applicable to
	// global
	// addresses.*
	...
	// being reserved. A RESERVED address is currently reserved
	// and
	...
	// can
	// only be used with INTERNAL type with aGCE_ENDPOINT or DNS_RESOLVER
	// purpose.
...
type AuthenticationPolicy struct {
	...
	// authentication.
	// Similar to peers, these will be evaluated in order the first valid
	// one
	// will be used to set origin identity. If none of these methods pass,
	// the
	...
	// authentication.
	// They will be evaluated in order the first valid one will be used to
	// set
	// peer identity. If none of these methods pass, the request will be
	// rejected
	// with authentication failed error (401). Leave the list empty if
	// peer
	...
	// available,
	// either because peer/origin authentication is not defined, or
	// failed,
	// principal will be left unset. In other words, binding rule does not
	// affect
	// the decision to accept or reject request. This field can be set to
	// one of
	// the following:
...
type Backend struct {
	// Specifies how to determine whether the backend of a load balancer
	// can
	...
	// balancing
	// modes, the configuration might be accepted even though it has no
	// impact
	...
	// 100% of its configured capacity (depending onbalancingMode). A
	// setting of 0 means the group is
	...
	//
	// Not available with backends that don't support using abalancingMode.
	// This includes backends such as global
	...
	// group (NEG) resource. To determine what types of backends a
	// load
	// balancer supports, see the [Backend
	// services
	// overview](https://cloud.google.com/load-balancing/docs/backen
	// d-service#backends).
	//
	...
	// Defines a maximum number of in-flight requests for a single
	// endpoint.
	...
	// Defines a maximum target for requests per second (RPS). For
	// usage
	// guidelines, seeRate
	// balancing mode and Utilization
	// balancing mode.
	...
	// Optional parameter to define a target capacity for theUTILIZATION
	...
	//    capacity, backends in this layer would be used and traffic would
	// be
	...
	// e.g.
	//
	// "run.googleapis.com/projects/123456789/locations/us-central1/services/
	// my-service"
	//  for Cloud Run service.
	//
	// "compute.googleapis.com/projects/123456789/regions/us-central1/service
	// Attachments/my-service-attachment"
...
type BackendCustomMetric struct {
	...
	// following
	// characters must be a dash, period, underscore, lowercase letter,
	// or
...
type BackendService struct {
	...
	// the end of the browser session (or equivalent). The maximum allowed
	// value
	...
	// A boolean flag enabling multi-network mesh. This field is only
	// allowed with
	// load balancing scheme set to INTERNAL_SELF_MANAGED.
	AllowMultinetwork bool `json:"allowMultinetwork,omitempty"`
	// The list of backends that serve this BackendService.
	Backends []*Backend `json:"backends,omitempty"`
	// Cloud CDN configuration for this BackendService. Only available
	// for
	...
	// Connection
	// tracking policy settings are only available for external
	// passthrough
	...
	// load
	// balancing policy is applicable only for HTTP connections. The
	// affinity to a
	...
	// [Creating
	// custom
	// headers](https://cloud.google.com/load-balancing/docs
	// /custom-headers).
	...
	// create the resource.
	Description string `json:"description,omitempty"`
	// Dynamic forwarding configuration. This field is used to configure
	// the
	...
	// must be
	// changed to PREPARE. The state must be changed to TEST_ALL_TRAFFIC
	// before
	// the loadBalancingScheme can be changed to EXTERNAL_MANAGED.
	// Optionally, the
	...
	// Load
	// Balancers](https://cloud.google.com/load-balancing/docs/internal/
	// failover-overview)
	// and [external passthrough Network
	// Load
	...
	// when
	// inserting a BackendService. An up-to-date fingerprint must be
	// provided in
	...
	//
	// When haPolicy is set for an Internal Passthrough Network Load
	// Balancer, the
	...
	//
	// When haPolicy is specified, the set of attached network endpoints
	// across
	// all backends comprise an High Availability domain from which one
	// endpoint
	// is selected as the active endpoint (the leader) that receives
	// all
	// traffic.
	//
	// haPolicy can be added only at backend service creation time. Once set
	// up,
	...
	// backend
	// service. Not all backend services support legacy health checks.
	// See
	// Load balancer guide. Currently, at most one health check can
	// be
	...
	// IPv4
	//    health checks are used to check the health of the backends. This
	// is the
	//    default setting.
	//    - PREFER_IPV6: Prioritize the connection to the endpoint's
	//    IPv6 address over its IPv4 address (provided there is a healthy
	// IPv6
	//    address).
	...
	//    Group), regardless of traffic from the client to the proxy. Only
	// IPv6
	//    health checks are used to check the health of the
	// backends.
	//
	//
	//
	// This field is applicable to either:
	//
	//    -  Advanced global external Application Load Balancer (load
	// balancing
	...
	//    -  Traffic
	//    Director with Envoy proxies and proxyless gRPC (load balancing
	// scheme
	//    INTERNAL_SELF_MANAGED).
	...
	//
	// Caution: This field and its children are intended for use in a
	// service mesh
	...
	// the
	// response has been completely processed, including all retries. A
	// stream
	...
	//
	// This value can be overridden in the PathMatcher configuration of
	// the
	...
	// Balancers when
	// the haPolicy is enabled, and for External Passthrough Network
	// Load
	// Balancers when the haPolicy fastIpMove is enabled.
	//
	// This field can only be specified when the load balancing scheme is
	// set toINTERNAL, or when the load balancing scheme is set toEXTERNAL
	...
	// considered disabled.
	//
	// Results of the outlier detection algorithm (ejection of endpoints
	// from the
	...
	//    - Serverless NEGs, that resolve to Cloud Run, App Engine, or
	// Cloud
	//    Functions Services
	//    - Private Service Connect NEGs, that resolve to
	//    Google-managed regional API endpoints or managed services
	// published using
	...
	// named port must be [defined on each backend
	// instance
	// group](https://cloud.google.com/load-balancing/docs/backend-s
	// ervice#named_ports).
	...
	// This field specifies the security settings that apply to this
	// backend
	...
	// see:
	// [Session
	// Affinity](https://cloud.google.com/load-balancing/docs/b
	// ackend-service#session_affinity).
	//
	...
	// The backend service timeout has a different meaning depending on
	// the
...
type BackendServiceCdnPolicy struct {
	...
	// e.g.
	// Pragma or Authorization headers. Up to 5 headers can be
	// specified.
	...
	// the
	// response max-age directive, along with a "public" directive.
	// For
	// cacheable content in CACHE_ALL_STATIC mode, client_ttl clamps the
	// max-age
	// from the origin (if specified), or else sets the response
	// max-age
	// directive to the lesser of the client_ttl and default_ttl, and
	// also
	// ensures a "public" cache-control directive is present.
	// If a client TTL is not specified, a default value (1 hour) will be
	// used.
	...
	// will overwrite the TTL set in all responses. The maximum allowed
	// value is
	...
	// than
	// this, or an Expires header more than maxTTL seconds in the future
	// will
	// be capped at the value of maxTTL, as if it were the value of
	// an
	// s-maxage Cache-Control directive.
	// Headers sent to the client will not be modified.
	// Setting a TTL of "0" means "always revalidate".
	// The maximum allowed value is 31,622,400s (1 year), noting
	// that
	// infrequently accessed objects may be evicted from the cache
	// before
	// the defined TTL.
	...
	// This can reduce the load on your origin and improve
	// end-user
	// experience by reducing response latency.
	// When the cache mode is set to CACHE_ALL_STATIC or
	// USE_ORIGIN_HEADERS,
	// negative caching applies to responses with the specified response
	// code
	// that lack any Cache-Control, Expires, or Pragma: no-cache
	// directives.
	// When the cache mode is set to FORCE_CACHE_ALL, negative caching
	// applies
	// to all responses with the specified response code, and override
	// any
	// caching headers.
	// By default, Cloud CDN will apply the following default TTLs to
	// these
	// status codes:
	...
	// Omitting the policy and leaving negative_caching enabled will
	// use
	// Cloud CDN's default cache TTLs.
	// Note that when specifying an explicit negative_caching_policy,
	// you
	...
	// revalidating
	// content with the origin, or when an error is encountered when
	// refreshing
	// the cache.
	...
	// limit
	// (max-stale) is 86400s (1 day), which will allow stale content to
	// be
	...
	// be
	// considered fresh. After this time period, the response will
	// be
	// revalidated before being served. Defaults to 1hr (3600s).  When
	// serving
	// responses to signed URL requests, Cloud CDN will internally behave
	// as
	// though all responses from this backend had a "Cache-Control:
	// public, max-age=[TTL]" header, regardless of any
	// existing
...
type BackendServiceCdnPolicyNegativeCachingPolicy struct {
	...
	// codes
	// 300, 301, 302, 307, 308, 404, 405, 410, 421, 451 and 501 can
	// be
	// specified as values, and you cannot specify a status code more
	// than
	// once.
	Code int64 `json:"code,omitempty"`
	// The TTL (in seconds) for which to cache responses with
	// the
	// corresponding status code.
	// The maximum allowed value is 1800s (30 minutes), noting
	// that
...
type BackendServiceConnectionTrackingPolicy struct {
	...
	// persist on unhealthy backends only for connection-oriented
	// protocols
	// (TCP and SCTP) and only if the Tracking Mode isPER_CONNECTION
	// (default tracking mode) or the Session
	// Affinity is configured for 5-tuple. They do not persist forUDP.
	//
	// If set to NEVER_PERSIST, after a backend becomes unhealthy,
	// the existing connections on the unhealthy backend are never persisted
	// on
	...
	// persist on unhealthy backends regardless of protocol and
	// session
	// affinity. It is generally not recommended to use this mode overriding
	// the
	// default.
	//
	// For more details, see [Connection Persistence for Network
	// Load
	// Balancing](https://cloud.google.com/load-balancing/docs/network/n
	// etworklb-backend-service#connection-persistence)
	// and [Connection Persistence for Internal TCP/UDP
	// Load
	// Balancing](https://cloud.google.com/load-balancing/docs/internal#
	// connection-persistence).
	ConnectionPersistenceOnUnhealthyBackends string `json:"connectionPersistenceOnUnhealthyBackends,omitempty"`
	// Enable Strong Session Affinity for external passthrough Network
	// Load
	...
	//
	//    - The minimum (default) is 10 minutes and the maximum is 16
	// hours.
	...
	//
	//
	// For external passthrough Network Load Balancers the default is
	// 60
	...
	//    the configured Session Affinity. It matches the configured
	// Session
	//    Affinity.
	//
	//
	//
	// For more details, see [Tracking Mode for Network
	// Load
	// Balancing](https://cloud.google.com/load-balancing/docs/network/n
	// etworklb-backend-service#tracking-mode)
	// and [Tracking Mode for Internal TCP/UDP
	// Load
	// Balancing](https://cloud.google.com/load-balancing/docs/internal#
	// tracking-mode).
...
// BackendServiceDynamicForwardingForwardProxy is a composite type wrapping the Alpha, Beta, and GA methods for its GCE equivalent
...
type BackendServiceFailoverPolicy struct {
	...
	// among all primary VMs when all primary and all backup backend VMs
	// are
	//  unhealthy.
	...
	// the load balancer performs a
	// failover when the number of healthy primary VMs equals zero.
	// For all other values, the load balancer performs a failover when
	// the
...
type BackendServiceHAPolicy struct {
	...
	// network
	//    endpoint as the leader. This method is faster than updating the
	// leader
	//    using the haPolicy.leader API. Fast IP move works as follows: The
	// VM
	//    hosting the network endpoint that should become the new leader
	// sends
	//    either a Gratuitous ARP (GARP) packet (IPv4) or an ICMPv6 Router
	//    Advertisement(RA) packet (IPv6).  Google Cloud immediately but
	//    temporarily associates the forwarding rule IP address with that
	// VM, and
	//    both new and in-flight packets are quickly delivered to that
	// VM.
	//
	//
	//
	...
	//
	//    - The GARP/RA-initiated re-routing stays active for approximately
	// 20
	//    minutes. After triggering fast failover, you must also
	//    appropriately set the haPolicy.leader.
	//    -  The new leader instance should continue to send GARP/RA
	// packets
	//    periodically every 10 seconds until at least 10 minutes after
	// updating
	//    the haPolicy.leader (but stop immediately if it is no longer the
	// leader).
	//    - After triggering a fast failover, we recommend that you wait at
	// least
	//    3 seconds before sending another GARP/RA packet from a different
	// VM
	//    instance to avoid race conditions.
	//    - Don't send GARP/RA packets from different VM
	//    instances at the same time. If multiple instances continue to
	// send
	//    GARP/RA packets, traffic might be routed to different destinations
	// in an
	...
	//    -  When multiple forwarding rules refer to a regional backend
	// service,
	//    you need only send a GARP or RA packet for a single forwarding
	// rule
	//    virtual IP. The virtual IPs for all forwarding rules targeting the
	// same
	//    backend service will also be moved to the sender of the GARP or
	// RA
	//    packet.
	//
	//
	//
	// The following are the Fast IP Move limitations (that is, when
	// fastIPMove
	// is not DISABLED):
	//
	//    - Multiple forwarding rules cannot use the same IP address if one
	// of
	//    them refers to a regional backend service with fastIPMove.
	//    - The regional backend service must set the network field, and
	// all
	//    NEGs must belong to that network. However, individual
	//    NEGs can belong to different subnetworks of that network.
	//    - The maximum number of network endpoints across all backends of
	// a
	//    backend service with fastIPMove is 32.
	//    - The maximum number of backend services with fastIPMove that can
	// have
	//    the same network endpoint attached to one of its backends is 64.
	//    - The maximum number of backend services with fastIPMove in a VPC
	// in a
	//    region is 64.
	//    - The network endpoints that are attached to a backend of a
	// backend
	//    service with fastIPMove cannot resolve to Gen3+ machines for
	// IPv6.
	//    - Traffic directed to the leader by a static route next hop will
	// not be
	//    redirected to a new leader by fast failover. Such traffic will
	// only be
	//    redirected once an haPolicy.leader update has taken effect. Only
	// traffic
	//    to the forwarding rule's virtual IP will be redirected to a new
	// leader by
	//    fast failover.
	//
	//
	// haPolicy.fastIPMove can be set only at backend service creation
	// time.
	// Once set, it cannot be updated.
	//
	// By default, fastIpMove is set to DISABLED.
	FastIPMove string `json:"fastIPMove,omitempty"`
	// Selects one of the network endpoints attached to the backend NEGs
	// of
	// this service as the active endpoint (the leader) that receives
	// all
	// traffic.
	//
	// When the leader changes, there is no connection draining to
	// persist
	// existing connections on the old leader.
	//
	// You are responsible for selecting a suitable endpoint as the
	// leader. For example, preferring a healthy endpoint over unhealthy
	// ones.
	// Note that this service does not track backend endpoint health,
	// and
...
type BackendServiceHAPolicyLeader struct {
	...
	// of the zonal Network Endpoint Group (NEG) with `GCE_VM_IP`
	// endpoints
	// that the leader is attached to.
	//
	// The leader's backendGroup must already be specified as a backend
	// of
	// this backend service. Removing a backend that is designated as
	// the
	// leader's backendGroup is not permitted.
	BackendGroup string `json:"backendGroup,omitempty"`
	// The network endpoint within the leader.backendGroup that
	// is
	// designated as the leader.
	//
	// This network endpoint cannot be detached from the NEG specified
	// in
	// the haPolicy.leader.backendGroup until the leader is updated
	// with
...
type BackendServiceIAPOAuth2ClientInfo struct {
	...
	// Optional - If not provided, the name will be autogenerated by
	// the
	// backend.
...
type BackendServiceLocalityLoadBalancingPolicyConfigCustomPolicy struct {
	...
	//
	// The value should match the name of a custom implementation
	// registered
	// on the gRPC clients. It should follow protocol buffer message
	// naming
	// conventions and include the full path (for
	// example,
...
type BackendServiceLocalityLoadBalancingPolicyConfigPolicy struct {
	...
	// include
	// ROUND_ROBIN and, for Java clients, LEAST_REQUEST. For
	// information
...
type BackendServiceLogConfig struct {
	...
	// backend
	// service. Configures whether all, none or a subset of optional
	// fields
...
type BackendServiceNetworkPassThroughLbTrafficPolicyZonalAffinity struct {
	...
	//    enabled. The load balancer distributes new connections to all
	// healthy
	//    backend endpoints in the local zone only. If there are no healthy
	//    backend endpoints in the local zone, the load balancer
	// distributes
	...
	//    distributes new connections to all healthy backend endpoints
	// across all
	//    zones.
	Spillover string `json:"spillover,omitempty"`
	// The value of the field must be in [0, 1]. When the ratio of the
	// count
	// of healthy backend endpoints in a zone to the count of
	// backend
	// endpoints in that same zone is equal to or above this threshold,
	// the
	// load balancer distributes new connections to all healthy endpoints
	// in
	// the local zone only. When the ratio of the count of healthy
	// backend
	// endpoints in a zone to the count of backend endpoints in that
	// same
...
type BackendServiceParams struct {
	...
	// only. The keys/values to set on the resource should be specified
	// in
	...
	// Note:
	// * Invalid combinations of ID & namespaced format is not supported.
	// For
...
type BackendServiceTlsSettings struct {
	...
	// use
	// certificates and roots of trust provisioned by the Managed
	// Workload
	// Identity system.
	//
	//  The `identity` property is the
	// fully-specified SPIFFE ID to use in the SVID presented by the
	// Load
	// Balancer Workload.
	//
	...
	// valid
	// Workload Identity Pool. The TrustConfig and
	...
	//
	//  The system will set up a read-onlytlsSettings.authenticationConfig
	...
	// to
	// the backend, and requires that this string match a Subject
	// Alternative
	// Name (SAN) in the backend's server certificate. With a Regional
	// Internet
	// NEG backend, if the SNI is specified here, the load balancer uses
	// it
	// regardless of whether the Regional Internet NEG is specified with
	// FQDN or
	// IP address and port. When both sni and subjectAltNames[] are
	// specified,
	...
	// Balancer
	// verifies during a TLS handshake with the backend. When the
	// server
	...
	// SAN
	// match one of the subjectAltNames in the list. This field is limited
	// to 5
	// entries. When both sni and subjectAltNames[] are specified, the
	// load
	// balancer matches the backend certificate's SAN only to
	// subjectAltNames[].
...
type BackendServiceUsedBy struct {
	// Output only. [Output Only] Server-defined URL for resources
	// referencing given
...
// CachePolicy is a composite type wrapping the Alpha, Beta, and GA methods for its GCE equivalent
type CachePolicy struct {
	// Bypass the cache when the specified request headers are matched by
	// name,
	// e.g. Pragma or Authorization headers. Values are case-insensitive. Up
	// to 5
	...
	// The cache key configuration. If not specified, the default behavior
	// depends
	// on the backend type: for Backend Services, the complete request URI
	// is
	// used; for Backend Buckets, the request URI is used without the
	// protocol or
	...
	// Specifies the cache setting for all responses from this route. If
	// not
	// specified, Cloud CDN uses `CACHE_ALL_STATIC` mode.
	CacheMode string `json:"cacheMode,omitempty"`
	// Specifies a separate client (e.g. browser client) maximum TTL for
	// cached
	...
	// `defaultTtl`
	// is used for the response max-age directive, along with a
	// "public"
	// directive. For cacheable content in `CACHE_ALL_STATIC` mode,
	// `clientTtl`
	// clamps the max-age from the origin (if specified), or else sets
	// the
	...
	// not
	// specified, Cloud CDN uses 3600s (1 hour) for `CACHE_ALL_STATIC`
	// mode.
	// Cannot exceed `maxTtl`. Cannot be specified when `cacheMode`
	// is
	// `USE_ORIGIN_HEADERS`.
	ClientTtl *Duration `json:"clientTtl,omitempty"`
	// Specifies the default TTL for cached content for responses that do
	// not have
	...
	// Specifies the maximum allowed TTL for cached content. Cache
	// directives that
	...
	// end-user
	// experience by reducing response latency.
	// When the `cacheMode` is set to `CACHE_ALL_STATIC`
	// or
	...
	// By
	// default, Cloud CDN applies the following TTLs to these HTTP status
	// codes:
	//
	...
	NegativeCaching bool `json:"negativeCaching,omitempty"`
	// Sets a cache TTL for the specified HTTP status
	// code.
	...
	// explicit
	// `negativeCachingPolicy`, you should take care to specify a cache TTL
	// for
...
// CachePolicyCacheKeyPolicy is a composite type wrapping the Alpha, Beta, and GA methods for its GCE equivalent
type CachePolicyCacheKeyPolicy struct {
	...
	//
	// Note: This field applies to routes that use backend services.
	// Attempting
	// to set it on a route that points exclusively to Backend Buckets
	// will
	// result in a configuration error. For routes that point to a
	// Backend
	// Bucket, use `includedQueryParameters` to define which parameters
	// should
	// be part of the cache key.
	ExcludedQueryParameters []string `json:"excludedQueryParameters,omitempty"`
	// If true, requests to different hosts will be cached
	// separately.
	//
	...
	// the
	// host is never included in a Backend Bucket's cache key. Attempting to
	// set
	// it on a route that points exclusively to Backend Buckets will result
	// in a
	// configuration error.
	IncludeHost bool `json:"includeHost,omitempty"`
	// If true, http and https requests will be cached separately.
	...
	// Service. It does not affect requests served by a Backend Bucket, as
	// the
	// protocol is never included in a Backend Bucket's cache key.
	// Attempting to
	// set on a route that points exclusively to Backend Buckets will result
	// in
	// a configuration error.
	IncludeProtocol bool `json:"includeProtocol,omitempty"`
	// If true, include query string parameters in the cache key according
	// to
	// `includedQueryParameters` and `excludedQueryParameters`. If
	// neither
	// is set, the entire query string will be included. If false, the
	// query
	...
	// Backend
	// Buckets will result in a configuration error. Up to 5 cookie names
	// can be
	// specified.
...
// CachePolicyNegativeCachingPolicy is a composite type wrapping the Alpha, Beta, and GA methods for its GCE equivalent
...
type CircuitBreakers struct {
	...
	// The maximum number of parallel requests that allowed to the
	// backend
	...
	// This parameter is respected by both the HTTP/1.1 and
	// HTTP/2
...
type ClientTlsSettings struct {
	...
	// and
	// identity information. This field is only applicable when mode is set
	// to
	// MUTUAL.
	...
	// TLS.
	// The value of this field determines how TLS is enforced. This can be
	// set
	// to one of the following values: DISABLE: Do not setup a TLS
	// connection to
	// the backends.
	...
	Sni string `json:"sni,omitempty"`
	// A list of alternate names to verify the subject identity in
	// the
	// certificate.If specified,
	// the proxy will verify that the server certificate's subject alt
	// name
	// matches one of the specified values. This field is applicable only
	// when
...
type ConsistentHashLoadBalancerSettings struct {
	...
	// will
	// be used as the hash key for the consistent hash load balancer. If
	// the
	// cookie is not present, it will be generated. This field is applicable
	// if
	...
	// Defaults to
	// 1024. Larger ring sizes result in more granular load distributions.
	// If the
...
type CorsPolicy struct {
	...
	// indicates that
	// the actual request can include user credentials. This field
	// translates to
	...
	//
	// An origin is allowed if it matches either an item inallowOrigins or
	// an item inallowOriginRegexes.
	//
...
type CustomErrorResponsePolicy struct {
	...
	//
	// In a given policy, if you specify rules for both a range of error
	// codes
	// as well as rules for specific error codes then rules with specific
	// error
	// codes have a higher priority. For example, assume that you configure
	// a rule
	// for 401 (Un-authorized) code, and another for all 4 series
	// error codes (4XX). If the backend service returns a401, then the rule
	// for 401 will be applied.
	// However if the backend service returns a 403, the rule for4xx takes
	// effect.
	...
	//
	// If errorService is not specified at lower levels likepathMatcher,
	// pathRule and routeRule,
	// an errorService specified at a higher level in theUrlMap will be
	// used. IfUrlMap.defaultCustomErrorResponsePolicy contains one or
	...
	// be returned, with the original response code (oroverrideResponseCode
	// if configured).
	//
...
type CustomErrorResponsePolicyCustomErrorResponseRule struct {
	...
	//      applies the policy if the error code exactly matches this
	// value.
	//      - 5xx: Load Balancer will apply the policy if the
	//      backend service responds with any response code in the range of
	//      500 to 599.
	//    - 4xx: Load
	//      Balancer will apply the policy if the backend service responds
	// with any
	...
	// load balancer cannot reach the BackendBucket, a simpleNot Found Error
...
type ForwardingRule struct {
	...
	//    - Set this field to true to allow packets addressed to any port
	// or
	//    packets lacking destination port information (for example, UDP
	// fragments
	//    after the first fragment) to be forwarded to the backends
	// configured with
	//    this forwarding rule. The L3_DEFAULT protocol requiresallPorts be
	// set to true.
	AllPorts bool `json:"allPorts,omitempty"`
	// If set to true, clients can access the internal passthrough Network
	// Load
	// Balancers, the regional internal Application Load Balancer, and
	// the
	...
	// to
	// this forwarding rule. Possible values are PREPARE,
	// TEST_BY_PERCENTAGE, and
	// TEST_ALL_TRAFFIC.
	//
	// To begin the migration from EXTERNAL to EXTERNAL_MANAGED, the state
	// must be
	...
	//
	// Rolling
	//  back a migration requires the states to be set in reverse order.
	// So
	// changing the scheme from EXTERNAL_MANAGED to EXTERNAL requires the
	// state to
	// be set to TEST_ALL_TRAFFIC at the same time. Optionally,
	...
	// Determines the fraction of requests to backend buckets that should
	// be
	...
	//
	//
	//    - When the target is set to targetGrpcProxy
	...
	// * Full resource URL, as
	// inhttps://www.googleapis.com/compute/v1/projects/project_id/regions/re
	// gion/addresses/address-name
	...
	// [IP
	// address
	// specifications](https://cloud.google.com/load-balancing/docs/f
	// orwarding-rule-concepts#ip_address_specifications).
	//
	...
	// products
	// as described in [Load
	// balancing
	// features](https://cloud.google.com/load-balancing/docs/featu
	...
	// Full resource URL, as
	// inhttps://www.googleapis.com/compute/v1/projects/project_id/regions/re
	// gion/publicDelegatedPrefixes/sub-pdp-name
	// Partial URL, as in:
	//
	//    -
	...
	// collector for
	// packet mirroring. To prevent mirroring loops, instances behind
	// this
	// load balancer will not have their traffic mirrored even if
	// aPacketMirroring rule applies to them.
	...
	Kind string `json:"kind,omitempty"`
	// A fingerprint for the labels being applied to this resource, which
	// is
	// essentially a hash of the labels set used for optimistic locking.
	// The
	// fingerprint is initially generated by Compute Engine and changes
	...
	// those proxies.
	//
	// For each metadataFilter in this list, if itsfilterMatchCriteria is
	// set to MATCH_ANY, at least one of thefilterLabels must match the
	// corresponding label provided in
	// the metadata. If its filterMatchCriteria is set to
	// MATCH_ALL, then all of its filterLabels must match with
	// corresponding labels provided in the metadata. If
	// multiplemetadataFilters are specified, all of them need to be
	// satisfied
	...
	// andSTANDARD. For GlobalForwardingRule, the valid value isPREMIUM.
	//
	// If this field is not specified, it is assumed to be PREMIUM.
	// If IPAddress is specified, this value must be equal to
	// the
	// networkTier of the Address.
	NetworkTier string `json:"networkTier,omitempty"`
	// This is used in PSC consumer ForwardingRule to control whether it
	// should
	// try to auto-generate a DNS zone or not. Non-PSC forwarding rules do
	// not use
	...
	// Balancers,
	//    internal and external Application Load Balancers, external
	// protocol
	...
	//    - You can specify a list of up to five ports by number, separated
	// by
	...
	// and
	// this field can only be used with a regional forwarding rule whose
	// scheme isEXTERNAL.
	// Each source_ip_range entry should be either an IP address
	// (for
	...
	// g-rule-concepts#ip_address_specifications).
	//      -  For Private Service Connect forwarding rules that forward
	// traffic to Google APIs, provide the name of a supported Google API
	// bundle:
	//
	//
...
// ForwardingRuleAttachedExtension is a composite type wrapping the Alpha, Beta, and GA methods for its GCE equivalent
...
type ForwardingRuleServiceDirectoryRegistration struct {
	...
	// APIs.
	// All PSC for Google APIs forwarding rules on the same network should
	// use
...
type GRPCHealthCheck struct {
	...
	// the
	// following values:
	// USE_FIXED_PORT: Specifies a port number explicitly using theport
	// field  in the health check. Supported by backend services
	...
	// supported
	// by backend services for proxy load balancers. Not supported by
	// target
	// pools.  Not supported by backend services for passthrough load
	// balancers.
	// Supports all backends that can be health checked; for
	// example,GCE_VM_IP_PORT network endpoint groups and instance
	// group
	// backends.
	//
	...
	// the
	// port number determined by looking up the backend service's named port
	// in
...
type HTTP2HealthCheck struct {
	...
	// see:
	// https://cloud.google.com/load-balancing/docs/health-check-concept
	// s#hc-packet-dest
...
type HTTPHealthCheck struct {
	...
	// for passthrough load balancers and backend services for proxy
	// load
	// balancers. Also supported in legacy HTTP health checks for target
	// pools.
...
type HealthCheck struct {
	...
	// dash,
	// lowercase letter, or digit, except the last character, which isn't a
	// dash.
	Name string `json:"name,omitempty"`
	// Output only. [Output Only] Region where the health check resides.
	// Not applicable to
	...
	// on what other health check fields are supported and what other
	// resources
...
type HostRule struct {
	...
	// ([a-z0-9-.]*). In
	// that case, * must be the first character, and if followed
	// by
...
type HttpFaultAbort struct {
	...
	// The percentage of traffic for connections, operations, or
	// requests
...
type HttpFilterConfig struct {
	// The configuration needed to enable thenetworkservices.HttpFilter
	// resource. The configuration must be
	// YAML formatted and only contain fields defined in the protobuf
	// identified
	// in configTypeUrl
...
type HttpHeaderMatch struct {
	...
	//
	// For matching against the HTTP request's authority, use a
	// headerMatch
	...
	// The value of the header must start with the contents
	// ofprefixMatch.
	//
	// Only one of exactMatch, prefixMatch,suffixMatch,
	// regexMatch,presentMatch or rangeMatch must be set.
	...
	// match takes place whether or not the request's header has a
	// value.
	//
	// Only one of exactMatch, prefixMatch,suffixMatch,
	// regexMatch,presentMatch or rangeMatch must be set.
	...
	// The value of the header must end with the contents
	// ofsuffixMatch.
	//
	// Only one of exactMatch, prefixMatch,suffixMatch,
	// regexMatch,presentMatch or rangeMatch must be set.
...
type HttpQueryParameterMatch struct {
	...
	// request contains the query parameter, irrespective of whether
	// the
	...
	PresentMatch bool `json:"presentMatch,omitempty"`
	// The queryParameterMatch matches if the value of the
	// parameter matches the regular expression specified byregexMatch.
	// For
	// more information about regular expression syntax, see Syntax.
	//
	// Only one of presentMatch, exactMatch, orregexMatch must be
	// set.
	//
	// Regular expressions can only be used when the loadBalancingScheme
	// is
...
type HttpRedirectAction struct {
	...
	//
	// This must only be set for URL maps used inTargetHttpProxys. Setting
	// this true forTargetHttpsProxy is not permitted.
	//
	// The default is set to false.
	HttpsRedirect bool `json:"httpsRedirect,omitempty"`
	// The path that is used in the redirect response instead of the
	// one
	// that was supplied in the request.
	//
	// pathRedirect cannot be supplied together withprefixRedirect. Supply
	// one alone or neither. If neither is
	// supplied, the path of the original request is used for the
	// redirect.
	//
	...
	//
	// prefixRedirect cannot be supplied together withpathRedirect. Supply
	// one alone or neither. If neither is
	// supplied, the path of the original request is used for the
	// redirect.
	//
...
type HttpRetryPolicy struct {
	...
	//
	// If not specified, will use the timeout set in theHttpRouteAction
	// field.
	...
	//
	// Only the following codes are supported when the URL map is bound
	// to
...
type HttpRouteAction struct {
	...
	// clients can
	// be aborted by the load balancer for a percentage of requests.timeout
	// and retry_policy is ignored by clients
	...
	//
	// If not specified, this field uses the maximummaxStreamDuration value
	// among all backend services associated
	// with the route.
	//
	...
	// balancer does
	// not wait for responses from the shadow service. Before sending
	// traffic
	...
	// from the
	// time the request has been fully processed (known as
	// *end-of-stream*)
	...
	// match
	// occurs. The weights determine the fraction of traffic that flows
	// to
	// their corresponding backend service. If all traffic needs to
	// go to a single backend service, there must be
	// oneweightedBackendService with weight set to a non-zero
	// number.
	//
...
type HttpRouteRule struct {
	...
	// Balancer returns error responses when BackendServiceorBackendBucket
	// responds with an error.
	//
	// If a policy for an
	// error code is not configured for the RouteRule, a policy for
	// the error code configured
	// inpathMatcher.defaultCustomErrorResponsePolicy is applied. If
	// one is not specified inpathMatcher.defaultCustomErrorResponsePolicy,
	// the policy
	...
	//      error code 404.
	//
	// If the request is for www.myotherdomain.com and a404 is encountered,
	// the policy underUrlMap.defaultCustomErrorResponsePolicy takes effect.
	// If a404 response is encountered for the
	// requestwww.example.com/current_events/, the pathMatcher's
	// policy
	...
	// error code, the policy atUrlMap.defaultCustomErrorResponsePolicy
	// takes effect.
	//
	// When
	...
	// afterpathMatchers[].routeRules[].routeAction.weightedBackendService.ba
	// ckendServiceWeightAction[].headerAction
	//
	...
	// Outbound route specific configuration fornetworkservices.HttpFilter
	// resources enabled by Traffic
	// Director. httpFilterConfigs only applies for load balancers
	// with loadBalancingScheme set toINTERNAL_SELF_MANAGED. See
	// ForwardingRule for
	// more details.
	//
	// Not supported when the URL map is bound to a target gRPC proxy
	...
	// Outbound route specific  metadata supplied
	...
	// The list of criteria for matching attributes of a request to
	// thisrouteRule. This list has OR semantics: the request matches
	// this routeRule when any of thematchRules are satisfied. However
	// predicates within
	...
	// priority determines the order in which a load balancer
	// interpretsrouteRules. RouteRules are evaluated in order
	// of priority, from the lowest to highest number. The priority of
	// a
	// rule decreases as its number increases (1, 2, 3, N+1). The first
	// rule
	...
	// 1, 2,
	// 3, 4, 5, 9, 12, 16 is a valid series of priority numbers to which
	// you
	...
	//
	//
	// Only one of urlRedirect, service orrouteAction.weightedBackendService
	// can be set.
	//
	//
	...
	// can be set.
	Service string `json:"service,omitempty"`
	// When this rule is matched, the request is redirected to a
	// URL
...
type HttpRouteRuleMatch struct {
	...
	//
	// Only one of prefixMatch, fullPathMatch,regexMatch or
	// path_template_match must be
	// specified.
	FullPathMatch string `json:"fullPathMatch,omitempty"`
	// Specifies a list of header match criteria, all of which must
	// match
	...
	// clients
	// present node
	// metadata. When there is a match, the relevant routing
	// configuration
	...
	// specified in ForwardingRule that refers to theUrlMap this
	// HttpRouteRuleMatch belongs to.
	//
	// metadataFilters only applies to load balancers that
	// haveloadBalancingScheme set toINTERNAL_SELF_MANAGED.
	//
	// Not supported when the URL map is bound to a target gRPC proxy
	// that
	// has validateForProxyless field set to true.
	MetadataFilters []*MetadataFilter `json:"metadataFilters,omitempty"`
	// If specified, this field defines a path template pattern that must
	// match
	// the :path header after the query string is removed.
	//
	// A path template pattern can include variables and
	// wildcards.
	...
	//       - No more than 5 operators (variables or wildcards) may appear
	// in
	//       the pattern.
	//
	// Precisely one ofprefixMatch, fullPathMatch,regexMatch, or
	// pathTemplateMatch must be
	// set.
	PathTemplateMatch string `json:"pathTemplateMatch,omitempty"`
	// For satisfying the matchRule condition, the request's
	// path must begin with the specified prefixMatch.prefixMatch must begin
	// with a /.
	//
	// The value must be from 1 to 1024 characters.
	//
	...
	// For satisfying the matchRule condition, the path of the
	// request must satisfy the regular expression specified inregexMatch
...
type Jwt struct {
	...
	// HTTP
	// request. If no explicit location is specified, the following
	// default
	...
	// extracted
	// from the locations its issuer specified or from the default
	// locations.
	//
	...
	// specifies
	// the query parameter name. For example, if jwt_params[0] is jwt_token,
	// the
...
type MetadataFilterLabelMatch struct {
	...
	//
	//  The name can have a maximum length of 1024 characters and must be
	// at
...
type MutualTls struct {
	...
	// STRICT: Client certificate must be presented, connection is in
	// TLS.
...
type NetworkEndpoint struct {
	...
	// Represents the port number to which PSC consumer sends
	// packets.
	//
	...
	// Optional, the field presence depends on the network endpoint type.
	// The
	...
	// zonal
	// NEGs) or in the zone within the region of the NEG (for regional
	// NEGs).
	// If the ipAddress is specified, it must belongs to the
	// VM
	// instance.
	//
	// The name must be 1-63 characters long, and comply withRFC1035
	...
	// address for
	// the VM instance in the network that the network endpoint group
	// belongs to
	// will be used.
	//
	// This field is redundant and need not be set for network endpoints of
	// typeGCE_VM_IP. If set, it must be set to the primary internal
	// IP
	// address of the attached VM instance that matches the subnetwork of
	// the NEG.
...
type NetworkEndpointGroup struct {
	...
	// group.
	Annotations map[string]string `json:"annotations,omitempty"`
	// Optional. Only valid when networkEndpointType isSERVERLESS. Only one
	...
	// endpoint.
	//
	// Optional. If the network endpoint type is either GCE_VM_IP,SERVERLESS
	// or PRIVATE_SERVICE_CONNECT, this
	...
	// the resource id.
	SelfLinkWithId string `json:"selfLinkWithId,omitempty"`
	// Only valid when networkEndpointType isSERVERLESS. Only one of
	// cloudRun,appEngine, cloudFunction orserverlessDeployment may be set.
	...
	// belong.
	Subnetwork string `json:"subnetwork,omitempty"`
	// Specify the type of this network endpoint group. OnlyLOAD_BALANCING
	// is valid for now.
	Type string `json:"type,omitempty"`
...
type NetworkEndpointGroupAppEngine struct {
	...
	// andfoo1-dot-appname.appspot.com/v2 can be backed by the
	// same
	// Serverless NEG with URL
	// mask<service>-dot-appname.appspot.com/<version>.
...
type NetworkEndpointGroupCloudFunction struct {
	...
	// mask allows for routing to multiple Cloud Functions without having
	// to
	// create multiple Network Endpoint Groups and backend services.
	//
	// For example, request URLs mydomain.com/function1
	// andmydomain.com/function2 can be backed by the same
...
type NetworkEndpointGroupCloudRun struct {
	...
	//
	// The service must be 1-63 characters long, and comply
	// withRFC1035.
	//
	...
	//
	// The tag must be 1-63 characters long, and comply
	// withRFC1035.
	//
	...
	// mask allows for
	// routing to multiple Run services without having to create
	// multiple
	// network endpoint groups and backend services.
	//
	// For example, request URLs foo1.domain.com/bar1
	// andfoo1.domain.com/bar2 can be backed by the same Serverless
	// Network Endpoint Group (NEG) with URL mask<tag>.domain.com/<service>.
	// The URL mask will
...
type NetworkEndpointGroupPscData struct {
	// Output only. [Output Only] Address allocated from given subnetwork
	// for PSC. This IP
	// address acts as a VIP for a PSC NEG, allowing it to act as an
	// endpoint in
	// L7 PSC-XLB.
	...
	// on
	// the PSC Producer side; should only be used for
	// the
	...
	// Consumer.
	PscConnectionId uint64 `json:"pscConnectionId,omitempty,string"`
	// Output only. [Output Only] The connection status of the PSC
	// Forwarding Rule.
	PscConnectionStatus string   `json:"pscConnectionStatus,omitempty"`
	ForceSendFields     []string `json:"-"`
	NullFields          []string `json:"-"`
...
type NetworkEndpointGroupServerlessDeployment struct {
	...
	// The user-defined name of the workload/instance. This value must
	// be
	...
	// backend
	// resources. The fields parsed by this template are platform-specific
	// and
	// are as follows:
	//
	//
...
type NetworkEndpointWithHealthStatus struct {
	...
	// Output only. [Output only] The health status of network
	// endpoint.
	//
...
type OutlierDetection struct {
	...
	// or 30s.
	//
	// After a backend endpoint is returned back to the load balancing pool,
	// it
	...
	// from the
	// load balancing pool. When the backend endpoint is accessed over HTTP,
	// a 5xx
	// return code qualifies as an error. Defaults to 5.
	ConsecutiveErrors int64 `json:"consecutiveErrors,omitempty"`
	// The number of consecutive gateway failures (502, 503, 504 status
	// or
	...
	// The percentage chance that a backend endpoint will be ejected when
	// an
	...
	// both new
	// ejections and backend endpoints being returned to service. The
	// interval is
	...
	// the mean
	// success rate, and the product of this factor and the standard
	// deviation of
...
type PathMatcher struct {
	...
	// policy has been defined for the error code at lower levels
	// likeRouteRule and PathRule within thisPathMatcher. If an error code
	// does not have a policy defined
	...
	//
	// When
	// used in conjunction withpathMatcher.defaultRouteAction.retryPolicy,
	// retries take
	// precedence. Only once all retries are exhausted,
	...
	//
	// Only one of defaultUrlRedirect, defaultService
	// or defaultRouteAction.weightedBackendService can be
	// set.
	//
	//
	...
	//
	// HeaderAction specified here are applied after the
	// matchingHttpRouteRule HeaderAction and before theHeaderAction in the
	// UrlMap
	//
	// HeaderAction is not supported for load balancers
	// that have
	...
	// before /a/b/* irrespective of the order in which those paths appear
	// in this
	// list.
	//
	// Within a given pathMatcher, only one ofpathRules or routeRules must
	// be set.
	PathRules []*PathRule `json:"pathRules,omitempty"`
	// The list of HTTP route rules. Use this list instead ofpathRules when
	// advanced route matching and routing actions are
	// desired. routeRules are evaluated in order of priority, from
	// the lowest to highest number.
	//
	// Within a given pathMatcher, you can set only one ofpathRules or
	// routeRules.
...
type PathRule struct {
	...
	// can be set.
	Service string `json:"service,omitempty"`
	// When a path pattern is matched, the request is redirected to a
	// URL
	// specified by urlRedirect.
	//
	//
...
type Permission struct {
	...
	// policy
	// applies to. Exact match, prefix match, and suffix match are
	// supported.
	Hosts []string `json:"hosts,omitempty"`
	// HTTP method.
...
type Principal struct {
	...
	// match, and
	// suffix match are supported for values. For
	// example,
...
type SSLHealthCheck struct {
	...
	// a
	// TCP connection and the TLS handshake, you can configure the health
	// check to
...
type SecuritySettings struct {
	...
	// methods can
	// be accepted on backends, and if authenticated, which
	// method/certificate
	...
	// authentication.
	// Allowed only for INTERNET_IP_PORT and INTERNET_FQDN_PORT NEG
	// backends.
	...
	// Optional. A list of Subject Alternative Names (SANs) that the
	// client
	...
	// to the client, the client inspects the certificate'ssubjectAltName
	// field. If the field contains one of the
	...
	//
	//  Only applies to a global BackendService withloadBalancingScheme set
	// to INTERNAL_SELF_MANAGED.
...
type SslCertificate struct {
	...
	// Output only. [Output Only] Domains associated with the certificate
	// via Subject
	// Alternative Name.
...
type SslCertificateManagedSslCertificate struct {
	// Output only. [Output only] Detailed statuses of the domains specified
	// for managed
	// certificate resource.
	DomainStatus map[string]string `json:"domainStatus,omitempty"`
	// The domains for which a managed SSL certificate will be generated.
	// Each
	// Google-managed SSL certificate supports up to the [maximum number
	// of
	// domains per Google-managed
	// SSL
	// certificate](/load-balancing/docs/quotas#ssl_certificates).
	Domains []string `json:"domains,omitempty"`
	// Output only. [Output only] Status of the managed certificate
	// resource.
	Status          string   `json:"status,omitempty"`
	ForceSendFields []string `json:"-"`
	NullFields      []string `json:"-"`
...
type SslCertificateSelfManagedSslCertificate struct {
	...
	// PEM format. The certificate chain must be no greater than 5
	// certs
	// long. The chain must include at least one intermediate cert.
	Certificate string `json:"certificate,omitempty"`
...
type Subsetting struct {
	...
	// Can only be set if `policy` is set to
...
type TargetHttpProxy struct {
	...
	// latest fingerprint, make a get() request to retrieve
	// the
	// TargetHttpProxy.
	Fingerprint string `json:"fingerprint,omitempty"`
	// URLs to networkservices.HttpFilter resources
	// enabled for xDS clients using this configuration. For
	// example,https://networkservices.googleapis.com/v1alpha1/projects/proje
	// ct/locations/locationhttpFilters/httpFilter
	// Only filters that handle outbound connection and
	// stream events may be specified. These filters work in conjunction
	// with
	...
	// these
	// filters within xDS configuration based on the name of the HTTP
	// filter. If
	...
	//
	//
	// httpFilters only applies for loadbalancers withloadBalancingScheme
	// set to INTERNAL_SELF_MANAGED.
	...
	// this
	// target proxy has a loadBalancingScheme set
	...
	// to
	// configure Envoy as a gateway or middle proxy (in other words, not
	// a
...
type TargetHttpsProxy struct {
	...
	// details.
	//
	//  authorizationPolicy only applies to a globalTargetHttpsProxy
	// attached toglobalForwardingRules with theloadBalancingScheme set to
	// INTERNAL_SELF_MANAGED.
	//
	...
	// This field can only be set for Global external Application Load
	// Balancer or
	...
	//
	//  Accepted format
	// is//certificatemanager.googleapis.com/projects/{project}/locations/{lo
	// cation}/certificateMaps/{resourceName}.
	...
	// request
	// will fail with error 412 conditionNotMet. To see the
	// latest
	...
	// URLs to networkservices.HttpFilter resources
	// enabled for xDS clients using this configuration. For
	// example,https://networkservices.googleapis.com/beta/projects/project/l
	// ocations/location/httpFilters/httpFilter
	// Only filters that handle outbound connection and
	// stream events may be specified. These filters work in conjunction
	// with
	// a default set of HTTP filters that may already be configured by
	...
	// resource. This
	// setting determines whether the load balancer attempts to negotiate
	// QUIC
	// with clients.
	...
	// regional forwardingRules with theloadBalancingScheme set to
	// EXTERNAL_MANAGED orINTERNAL_MANAGED. For details whichServerTlsPolicy
	// resources are accepted withINTERNAL_SELF_MANAGED and which with
	// EXTERNAL,INTERNAL_MANAGED, EXTERNAL_MANAGEDloadBalancingScheme
	...
	// include
	// the certificatemanager API namespace. Using Certificate
	// Manager
	...
	//
	//    -
	// //certificatemanager.googleapis.com/projects/{project}/locations/{loca
	// tion}/certificates/{resourceName}.
	//    -
	...
	//
	//
	// Requests with Early Data will have the "Early-Data" HTTP header set
	// on
	...
	// Applications
	// that allow idempotent HTTP methods to make non-idempotent changes,
	// such as
	...
	// defines
	// the mapping from URL to the BackendService. For example, the
	// following are
...
type TlsCertificateContext struct {
	// Specifies the certificate and private key paths. This field
	// is
	...
	// Specifies the config to retrieve certificates through SDS. This
	// field
...
type TlsContext struct {
	...
	// to
	// validate the client/server certificate. If omitted, the proxy will
	// not
...
type TlsValidationContext struct {
	// The path to the file holding the CA certificate to validate
	// the
...
type UrlMap struct {
	...
	// been
	// defined for the error code at lower levels like PathMatcher,
	// RouteRule and
	...
	// When the request for http://www.example.com/ encounters a502, the
	// policy inUrlMap.defaultCustomErrorResponsePolicy will be enforced.
	// When
	...
	// takes effect.
	//
	// When used in conjunction withdefaultRouteAction.retryPolicy, retries
	// take precedence. Only
	// once all retries are exhausted, thedefaultCustomErrorResponsePolicy
	// is applied. While attempting
	// a retry, if load balancer is successful in reaching the
	// service, the defaultCustomErrorResponsePolicy is ignored and
	// the response from the service is returned to the
	// client.
	...
	// for the selected backendService.
	//
	// The headerAction specified here take effect afterheaderAction
	...
	// Output only. [Output Only] URL of the region where the regional URL
	// map resides.
...
type UrlMapTest struct {
	...
	// matches the URL in the load balancer's
	// redirect response. If urlRedirect specifieshttps_redirect, the test
	// passes only if the scheme inexpectedOutputUrl is also set to
	// HTTPS.
	// If urlRedirect specifies strip_query, the test
	// passes only if expectedOutputUrl does not contain any
	// query
	// parameters.
	//
	...
	// ifexpectedRedirectResponseCode matches the HTTP status code in
	...
	// The expected URL that should  be redirected to for the host and
	// path
	// being tested.
	// [Deprecated] This field is deprecated.
...
type UrlRewrite struct {
	...
	// the
	// matching portion of the request's path is replaced
	// bypathPrefixRewrite.
	//
	// The value must be from 1 to 1024 characters.
	...
	// could be rewritten as /static/content/{format} to prefix/content to
	// the URL. Variables can also be re-ordered in a
	// rewrite, so that /{country}/{format}/{suffix=**} can be
	// rewritten as /content/{format}/{country}/{suffix}.
	//
	// At least
	// one non-empty routeRules[].matchRules[].path_template_match
	// is
	// required.
	//
...
type WeightedBackendService struct {
	...
	//
	// headerAction specified here take effect beforeheaderAction in the
	...
	// traffic.
	// Once a user's request has been directed to a backend
	// service,
	// subsequent requests are sent to the same backend service
	// as
	// determined by the backend service's session affinity policy.
	// Don't configure session affinity if you're using weighted
	// traffic
...
func ListAddresses(gceCloud *gce.Cloud, key *meta.Key, version meta.Version, logger klog.Logger, filter *filter.F) ([]*Address, error) {
	ctx, cancel := cloudprovider.ContextWithCallTimeout()
	defer cancel()
	...
	case meta.VersionAlpha:
		...
		case meta.Regional:
			...
			gceObjs, err = gceCloud.Compute().AlphaAddresses().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing alpha Address")
			gceObjs, err = gceCloud.Compute().AlphaGlobalAddresses().List(ctx, filter)
		}
	case meta.VersionBeta:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing beta region Address")
			gceObjs, err = gceCloud.Compute().BetaAddresses().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing beta Address")
			gceObjs, err = gceCloud.Compute().BetaGlobalAddresses().List(ctx, filter)
		}
	default:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing ga region Address")
			gceObjs, err = gceCloud.Compute().Addresses().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing ga Address")
			gceObjs, err = gceCloud.Compute().GlobalAddresses().List(ctx, filter)
		}
	}
	err = mc.Observe(err)
	if err != nil {
...
func ListBackendServices(gceCloud *gce.Cloud, key *meta.Key, version meta.Version, logger klog.Logger, filter *filter.F) ([]*BackendService, error) {
	ctx, cancel := cloudprovider.ContextWithCallTimeout()
	defer cancel()
	...
	case meta.VersionAlpha:
		...
		case meta.Regional:
			...
			gceObjs, err = gceCloud.Compute().AlphaRegionBackendServices().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing alpha BackendService")
			gceObjs, err = gceCloud.Compute().AlphaBackendServices().List(ctx, filter)
		}
	case meta.VersionBeta:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing beta region BackendService")
			gceObjs, err = gceCloud.Compute().BetaRegionBackendServices().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing beta BackendService")
			gceObjs, err = gceCloud.Compute().BetaBackendServices().List(ctx, filter)
		}
	default:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing ga region BackendService")
			gceObjs, err = gceCloud.Compute().RegionBackendServices().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing ga BackendService")
			gceObjs, err = gceCloud.Compute().BackendServices().List(ctx, filter)
		}
	}
	err = mc.Observe(err)
	if err != nil {
...
func ListForwardingRules(gceCloud *gce.Cloud, key *meta.Key, version meta.Version, logger klog.Logger, filter *filter.F) ([]*ForwardingRule, error) {
	ctx, cancel := cloudprovider.ContextWithCallTimeout()
	defer cancel()
	...
	case meta.VersionAlpha:
		...
		case meta.Regional:
			...
			gceObjs, err = gceCloud.Compute().AlphaForwardingRules().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing alpha ForwardingRule")
			gceObjs, err = gceCloud.Compute().AlphaGlobalForwardingRules().List(ctx, filter)
		}
	case meta.VersionBeta:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing beta region ForwardingRule")
			gceObjs, err = gceCloud.Compute().BetaForwardingRules().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing beta ForwardingRule")
			gceObjs, err = gceCloud.Compute().BetaGlobalForwardingRules().List(ctx, filter)
		}
	default:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing ga region ForwardingRule")
			gceObjs, err = gceCloud.Compute().ForwardingRules().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing ga ForwardingRule")
			gceObjs, err = gceCloud.Compute().GlobalForwardingRules().List(ctx, filter)
		}
	}
	err = mc.Observe(err)
	if err != nil {
...
func ListHealthChecks(gceCloud *gce.Cloud, key *meta.Key, version meta.Version, logger klog.Logger, filter *filter.F) ([]*HealthCheck, error) {
	ctx, cancel := cloudprovider.ContextWithCallTimeout()
	defer cancel()
	...
	case meta.VersionAlpha:
		...
		case meta.Regional:
			...
			gceObjs, err = gceCloud.Compute().AlphaRegionHealthChecks().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing alpha HealthCheck")
			gceObjs, err = gceCloud.Compute().AlphaHealthChecks().List(ctx, filter)
		}
	case meta.VersionBeta:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing beta region HealthCheck")
			gceObjs, err = gceCloud.Compute().BetaRegionHealthChecks().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing beta HealthCheck")
			gceObjs, err = gceCloud.Compute().BetaHealthChecks().List(ctx, filter)
		}
	default:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing ga region HealthCheck")
			gceObjs, err = gceCloud.Compute().RegionHealthChecks().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing ga HealthCheck")
			gceObjs, err = gceCloud.Compute().HealthChecks().List(ctx, filter)
		}
	}
	err = mc.Observe(err)
	if err != nil {
...
func ListNetworkEndpointGroups(gceCloud *gce.Cloud, key *meta.Key, version meta.Version, logger klog.Logger, filter *filter.F) ([]*NetworkEndpointGroup, error) {
	ctx, cancel := cloudprovider.ContextWithCallTimeout()
	defer cancel()
	...
	case meta.VersionAlpha:
		...
		gceObjs, err = gceCloud.Compute().AlphaNetworkEndpointGroups().List(ctx, key.Zone, filter)
	case meta.VersionBeta:
		logger.Info("Listing beta zoneNetworkEndpointGroup")
		gceObjs, err = gceCloud.Compute().BetaNetworkEndpointGroups().List(ctx, key.Zone, filter)
	default:
		logger.Info("Listing ga zoneNetworkEndpointGroup")
		gceObjs, err = gceCloud.Compute().NetworkEndpointGroups().List(ctx, key.Zone, filter)
	}
	err = mc.Observe(err)
	if err != nil {
		return nil, err
...
func ListSslCertificates(gceCloud *gce.Cloud, key *meta.Key, version meta.Version, logger klog.Logger, filter *filter.F) ([]*SslCertificate, error) {
	ctx, cancel := cloudprovider.ContextWithCallTimeout()
	defer cancel()
	...
	case meta.VersionAlpha:
		...
		case meta.Regional:
			...
			gceObjs, err = gceCloud.Compute().AlphaRegionSslCertificates().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing alpha SslCertificate")
			gceObjs, err = gceCloud.Compute().AlphaSslCertificates().List(ctx, filter)
		}
	case meta.VersionBeta:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing beta region SslCertificate")
			gceObjs, err = gceCloud.Compute().BetaRegionSslCertificates().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing beta SslCertificate")
			gceObjs, err = gceCloud.Compute().BetaSslCertificates().List(ctx, filter)
		}
	default:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing ga region SslCertificate")
			gceObjs, err = gceCloud.Compute().RegionSslCertificates().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing ga SslCertificate")
			gceObjs, err = gceCloud.Compute().SslCertificates().List(ctx, filter)
		}
	}
	err = mc.Observe(err)
	if err != nil {
...
func ListTargetHttpProxies(gceCloud *gce.Cloud, key *meta.Key, version meta.Version, logger klog.Logger, filter *filter.F) ([]*TargetHttpProxy, error) {
	ctx, cancel := cloudprovider.ContextWithCallTimeout()
	defer cancel()
	...
	case meta.VersionAlpha:
		...
		case meta.Regional:
			...
			gceObjs, err = gceCloud.Compute().AlphaRegionTargetHttpProxies().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing alpha TargetHttpProxy")
			gceObjs, err = gceCloud.Compute().AlphaTargetHttpProxies().List(ctx, filter)
		}
	case meta.VersionBeta:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing beta region TargetHttpProxy")
			gceObjs, err = gceCloud.Compute().BetaRegionTargetHttpProxies().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing beta TargetHttpProxy")
			gceObjs, err = gceCloud.Compute().BetaTargetHttpProxies().List(ctx, filter)
		}
	default:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing ga region TargetHttpProxy")
			gceObjs, err = gceCloud.Compute().RegionTargetHttpProxies().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing ga TargetHttpProxy")
			gceObjs, err = gceCloud.Compute().TargetHttpProxies().List(ctx, filter)
		}
	}
	err = mc.Observe(err)
	if err != nil {
...
func ListTargetHttpsProxies(gceCloud *gce.Cloud, key *meta.Key, version meta.Version, logger klog.Logger, filter *filter.F) ([]*TargetHttpsProxy, error) {
	ctx, cancel := cloudprovider.ContextWithCallTimeout()
	defer cancel()
	...
	case meta.VersionAlpha:
		...
		case meta.Regional:
			...
			gceObjs, err = gceCloud.Compute().AlphaRegionTargetHttpsProxies().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing alpha TargetHttpsProxy")
			gceObjs, err = gceCloud.Compute().AlphaTargetHttpsProxies().List(ctx, filter)
		}
	case meta.VersionBeta:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing beta region TargetHttpsProxy")
			gceObjs, err = gceCloud.Compute().BetaRegionTargetHttpsProxies().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing beta TargetHttpsProxy")
			gceObjs, err = gceCloud.Compute().BetaTargetHttpsProxies().List(ctx, filter)
		}
	default:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing ga region TargetHttpsProxy")
			gceObjs, err = gceCloud.Compute().RegionTargetHttpsProxies().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing ga TargetHttpsProxy")
			gceObjs, err = gceCloud.Compute().TargetHttpsProxies().List(ctx, filter)
		}
	}
	err = mc.Observe(err)
	if err != nil {
...
func ListUrlMaps(gceCloud *gce.Cloud, key *meta.Key, version meta.Version, logger klog.Logger, filter *filter.F) ([]*UrlMap, error) {
	ctx, cancel := cloudprovider.ContextWithCallTimeout()
	defer cancel()
	...
	case meta.VersionAlpha:
		...
		case meta.Regional:
			...
			gceObjs, err = gceCloud.Compute().AlphaRegionUrlMaps().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing alpha UrlMap")
			gceObjs, err = gceCloud.Compute().AlphaUrlMaps().List(ctx, filter)
		}
	case meta.VersionBeta:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing beta region UrlMap")
			gceObjs, err = gceCloud.Compute().BetaRegionUrlMaps().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing beta UrlMap")
			gceObjs, err = gceCloud.Compute().BetaUrlMaps().List(ctx, filter)
		}
	default:
		switch key.Type() {
		case meta.Regional:
			logger.Info("Listing ga region UrlMap")
			gceObjs, err = gceCloud.Compute().RegionUrlMaps().List(ctx, key.Region, filter)
		default:
			logger.Info("Listing ga UrlMap")
			gceObjs, err = gceCloud.Compute().UrlMaps().List(ctx, filter)
		}
	}
	err = mc.Observe(err)
	if err != nil {
