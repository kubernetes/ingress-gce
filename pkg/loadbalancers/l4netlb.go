...
func NewL4NetLB(service *corev1.Service, recorderFactory events.RecorderProducer, cloud *gce.Cloud, scope meta.KeyType, namer namer.L4ResourcesNamer, recorder record.EventRecorder) *L4NetLB {
	l4netlb := &L4NetLB{cloud: cloud,
		...
		l4HealthChecks: healthchecks.GetL4HealthChecksInstance(),
...
func (l4netlb *L4NetLB) syncDeleteHealthChecks() {
	...
	for _, isShared := range []bool{true, false} {
		err := l4netlb.l4HealthChecks.DeleteHealthCheck(l4netlb.Service, l4netlb.namer, isShared, meta.Regional, utils.XLB)
		if err != nil {
			l4netlb.SyncResult.GCEResourceInError = err.GCEResourceInError
			l4netlb.SyncResult.Error = err.Err
...
func (l4netlb *L4NetLB) syncHealthCheck(nodeNames []string) string {
	...
	hcResult, err := l4netlb.l4HealthChecks.EnsureHealthCheck(l4netlb.Service, l4netlb.namer, sharedHC, l4netlb.scope, utils.XLB, nodeNames)

	if err != nil {
		l4netlb.SyncResult.GCEResourceInError = annotations.HealthcheckResource
		l4netlb.SyncResult.Error = fmt.Errorf("Failed to ensure health check %s - %w", hcResult.HCName, err)
		return ""
	}
