...
func TestEnsureExternalIPv6OnlyLoadBalancer(t *testing.T) {
	t.Parallel()
	nodeNames := []string{"test-node-1"}
	...
	for _, tc := range testCases {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			svc := test.NewL4NetLBRBSDualStackService(protocol, tc.ipFamilies, tc.trafficPolicy)
			...
			if deleteResult.Error != nil {
				t.Errorf("EnsureLoadBalancerDeleted() resulted in error: %v", deleteResult.Error)
			}
			l4NetLB.EnsureLoadBalancerDeleted(l4NetLB.Service)
			assertDualStackNetLBResourcesDeleted(t, l4NetLB)
		})
...
func TestIPv6OnlyNetLBServiceUpdates(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range testCases {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			svc := test.NewL4NetLBRBSDualStackService(tc.initialProtocol, ipFamily, tc.initialTrafficPolicy)
			l4NetLB := mustSetupNetLBTestHandler(t, svc, nodeNames, false, true)
			...
			finalSvc := test.NewL4NetLBRBSDualStackService(tc.finalProtocol, ipFamily, tc.finalTrafficPolicy)
			finalSvc.Annotations = svc.Annotations
			l4NetLB.Service = finalSvc
...
func IPv6OnlyNetLBTransitionTestDesc(ipFamily []v1.IPFamily, initialTrafficPolicy v1.ServiceExternalTrafficPolicyType, finalTrafficPolicy v1.ServiceExternalTrafficPolicyType, initialProtocol v1.Protocol, finalProtocol v1.Protocol) string {
	fromTrafficPolicy := string(initialTrafficPolicy)
	toTrafficPolicy := string(finalTrafficPolicy)
	fromProtocol := string(initialProtocol)
	toProtocol := string(finalProtocol)
...
func TestIPv6OnlyNetLBNetworkTier(t *testing.T) {
	t.Parallel()
	nodeNames := []string{"test-node-1"}
	...
	testCases := []struct {
		...
		{
			desc:        "IPv6-only with no NetworkTier annotation (defaults to Premium)",
			ipFamilies:  []v1.IPFamily{v1.IPv6Protocol},
			networkTier: "",
			expectError: false,
		},
...
func TestIPv6OnlyNetLBStaticIPAnnotation(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.expectError {
				if result.Error == nil {
					t.Fatalf("EnsureFrontend(%+v) = nil, want error", svc.Spec)
				}
				t.Logf("EnsureFrontend(%+v) returned expected error: %v", svc.Spec, result.Error)
				return
			}
			svc.Annotations = result.Annotations
...
func TestIPv6OnlyNetLBBadCustomSubnet(t *testing.T) {
	t.Parallel()
	nodeNames := []string{"test-node-1"}
...
func mustSetupNetLBTestHandler(t *testing.T, svc *v1.Service, nodeNames []string, enableDualStack bool, enableIPv6Only bool) *L4NetLB {
	t.Helper()
...
func assertNetLBResourcesIPv6Only(t *testing.T, l4NetLB *L4NetLB, nodeNames []string) {
	...
	if err := verifyNetLBIPv4ResourcesDeletedOnSync(l4NetLB); err != nil {
		t.Errorf("verifyNetLBIPv4ResourcesDeletedOnSync(_) returned error %v, want nil (no IPv4 resources)", err)
	}
...
func buildExpectedNetLBAnnotations(l4netlb *L4NetLB) map[string]string {
	...
	if !l4netlb.enableIPv6Only && utils.NeedsIPv4(l4netlb.Service) {
		hcFwName := l4netlb.namer.L4HealthCheckFirewall(l4netlb.Service.Namespace, l4netlb.Service.Name, isSharedHC)
