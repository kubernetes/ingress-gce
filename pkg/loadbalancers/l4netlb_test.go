...
func TestEnsureL4NetLoadBalancer(t *testing.T) {
	...
	l4netlb := NewL4NetLB(svc, fakeGCE, meta.Regional, namer, record.NewFakeRecorder(100), healthchecks.NewL4(fakeGCE, &test.FakeRecorderSource{}))
	l4netlb.l4HealthChecks = healthchecks.NewL4(fakeGCE, &test.FakeRecorderSource{})
...
func TestHealthCheckFirewallDeletionWithILB(t *testing.T) {
	...
	// Create ILB service
	_, l4ilb, ilbResult := ensureService(fakeGCE, namer, nodeNames, vals.ZoneName, 8081, t)
	...
	l4NetLB := NewL4NetLB(netlbSvc, fakeGCE, meta.Regional, namer, record.NewFakeRecorder(100), l4ilb.l4HealthChecks)
...
func ensureNetLBResourceDeleted(t *testing.T, apiService *v1.Service, l4NetLb *L4NetLB) {
	...
	for _, fwName := range []string{resourceName, hcFwName} {
		...
		if err == nil || !utils.IsNotFoundError(err) {
			t.Errorf("Firewall rule %q should be deleted", fwName)
		}
	}
