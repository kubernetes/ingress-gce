...
// The string returned is the reserved IP address and bool indicating if IP address is managed by controller.
func (am *addressManager) HoldAddress() (string, bool, error) {
...
// bool indicating whether ip address is managed by controller and error.
func (am *addressManager) ensureAddressReservation() (string, bool, error) {
	...
	if am.targetIP == "" {
		return "", false, fmt.Errorf("failed to reserve address %q with no specific IP, err: %v", am.name, reserveErr)
	}
	...
	if err != nil {
		return "", false, fmt.Errorf("failed to get address by IP %q after reservation attempt, err: %q, reservation err: %q", am.targetIP, err, reserveErr)
	}
