...
func GetL4LoggingConfig(service *corev1.Service, configMapLister cache.Store) (*composite.BackendServiceLogConfig, error) {
	loggingConfigName, ok := annotations.FromService(service).GetL4ConfigMapAnnotation()
	if !ok {
		return nil, nil
	}
...
func parseL4LoggingEnabled(configMap *corev1.ConfigMap) (bool, error) {
	...
	return false, fmt.Errorf("unexpected value of enabled field: \"%s\", valid values are: \"%s\", \"%s\".", val, LoggingEnabledValue, LoggingDisabledValue)
}

// parseL4LoggingSampleRate parses sample rate. Should be float within [0, 1] range.
func parseL4LoggingSampleRate(configMap *corev1.ConfigMap) (float64, error) {
	...
	if err != nil {
		return 1, fmt.Errorf("invalid sample rate: \"%s\" - should be a float within [0.0, 1.0] range", strVal)
	}

	if val < 0 || val > 1 {
		return 1, fmt.Errorf("invalid sample rate: \"%s\" - should be within [0.0, 1.0] range", strVal)
	}
...
func parseL4LoggingOptionalMode(configMap *corev1.ConfigMap) (string, error) {
	...
	availableOptions := []string{
		OptionalModeExcludeAll,
		OptionalModeIncludeAll,
		OptionalModeCustom,
	}
	...
	return "", fmt.Errorf("invalid optional mode: \"%s\", available options are: %v", val, availableOptions)
}
