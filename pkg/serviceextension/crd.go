	...
	return &apiextensionsv1beta1.CustomResourceDefinition{
		...
		Spec: apiextensionsv1beta1.CustomResourceDefinitionSpec{
			...
			Names: apiextensionsv1beta1.CustomResourceDefinitionNames{
				...
				ListKind: "ServiceExtensionList",
				Plural:   "serviceextensions",
				Singular: "serviceextension",
			},
		},
	}
}
...
func EnsureCRD(clientset crdclient.Interface) (*apiextensionsv1beta1.CustomResourceDefinition, error) {
	...
	// to be created. Keeps watching the Established condition of ServiceExtension
	// CRD to be true.
	if err := wait.PollImmediate(checkCRDEstablishedInterval, checkCRDEstablishedTimeout, func() (bool, error) {
		...
		return nil, fmt.Errorf("timed out waiting for ServiceExtension CRD to become Established: %v", err)
	}

	glog.V(0).Infof("ServiceExtension CRD is Established.")
	return crd, nil
}
...
func createOrUpdateCRD(clientset crdclient.Interface) (*apiextensionsv1beta1.CustomResourceDefinition, error) {
	...
	if err != nil && !apierrors.IsNotFound(err) {
		return nil, fmt.Errorf("failed to verify the existence of ServiceExtension CRD: %v", err)
	}

	// Update ServieExtension CRD if already present.
	if err == nil {
		glog.V(0).Infof("Updating existing ServiceExtension CRD...")
		crd.ResourceVersion = existingCRD.ResourceVersion
		return clientset.ApiextensionsV1beta1().CustomResourceDefinitions().Update(crd)
	}

	glog.V(0).Infof("Creating ServiceExtension CRD...")
