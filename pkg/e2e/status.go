...
func (sm *StatusManager) init() error {
	...
	if err := sm.f.Clientset.CoreV1().ConfigMaps("default").Delete(context.TODO(), sm.cm.Name, metav1.DeleteOptions{}); !errors.IsNotFound(err) {
		klog.V(3).Infof("Error deleting config map %s: %v", sm.cm.Name, err)
	}

	sm.cm, err = sm.f.Clientset.CoreV1().ConfigMaps("default").Create(context.TODO(), sm.cm, metav1.CreateOptions{})
	if err != nil {
...
func (sm *StatusManager) shutdown() {
	...
	if err := sm.f.Clientset.CoreV1().ConfigMaps("default").Delete(context.TODO(), configMapName, metav1.DeleteOptions{}); err != nil {
...
func (sm *StatusManager) flush() {
	...
	updatedCm, err := sm.f.Clientset.CoreV1().ConfigMaps("default").Get(context.TODO(), configMapName, metav1.GetOptions{})
	...
	_, err = sm.f.Clientset.CoreV1().ConfigMaps("default").Update(context.TODO(), sm.cm, metav1.UpdateOptions{})
	if err != nil {
