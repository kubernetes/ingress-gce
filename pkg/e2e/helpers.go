...
func WaitForIngressDeletion(ctx context.Context, g *fuzz.GCLB, s *Sandbox, ing *networkingv1.Ingress, options *fuzz.GCLBDeleteOptions) error {
	crud := adapter.IngressCRUD{C: s.f.Clientset}
	if err := crud.Delete(ing.Namespace, ing.Name); err != nil && !apierrors.IsNotFound(err) {
...
func WaitForNegStatus(s *Sandbox, name string, expectSvcPorts []string, noPresentTest bool) (*negannotation.NegStatus, error) {
	var ret negannotation.NegStatus
	var err error
...
func CheckNegStatus(svc *v1.Service, expectSvcPorts []string) (negannotation.NegStatus, error) {
	annotation, ok := svc.Annotations[negannotation.NEGStatusKey]
	if !ok {
		return negannotation.NegStatus{}, fmt.Errorf("service %s/%s does not have neg status annotation: %v", svc.Namespace, svc.Name, svc)
	}
...
// The parameter expectedNegAttrs will map a port to a neg name. If the neg name is empty, CheckNameInNegStatus expects
// that the name is autogenerated and will check it
func CheckNameInNegStatus(svc *v1.Service, expectedNegAttrs map[string]string) (negannotation.NegStatus, error) {
	annotation, ok := svc.Annotations[negannotation.NEGStatusKey]
	if !ok && len(expectedNegAttrs) > 0 {
		return negannotation.NegStatus{}, fmt.Errorf("service %s/%s does not have neg status annotation: %+v", svc.Namespace, svc.Name, svc)
	} else if ok && len(expectedNegAttrs) == 0 {
		return negannotation.NegStatus{}, fmt.Errorf("service %s/%s should not have neg status annotation: %+v", svc.Namespace, svc.Name, svc)
	} else if !ok && len(expectedNegAttrs) == 0 {
		return negannotation.NegStatus{}, nil
	}
	...
	for port, negName := range negStatus.NetworkEndpointGroups {
		...
		if !ok {
			...
			return negannotation.NegStatus{}, fmt.Errorf("CheckCustomNegNameStatus: Service: %s, did not expect NEG with port %s", svc.Name, port)
		}
		if expectedName != "" && negName != expectedName {
			return negannotation.NegStatus{}, fmt.Errorf("CheckCustomNegNameStatus: Service: %s, with port %s was expected to have name %s", svc.Name, port, expectedName)
		}
		existingPorts += 1
	}
...
func WaitDestinationRuleAnnotation(s *Sandbox, namespace, name string, negCount int, timeout time.Duration) (*annotations.DestinationRuleNEGStatus, error) {
	...
	if err := wait.Poll(5*time.Second, timeout, func() (bool, error) {
		...
		if ann != nil {
			if val, ok := ann[negannotation.NEGStatusKey]; ok {
...
func WaitForNegCRs(s *Sandbox, serviceName string, expectedNegs map[string]string) (negannotation.NegStatus, error) {
	var svc *v1.Service
	...
	err := wait.Poll(negPollInterval, negPollTimeout, func() (bool, error) {
		...
		if len(expectedNegs) > 0 {
			statusWithZones, zoneErr := populateZonesFromCR(serviceName, svcNegs.Items, negannotation.NegStatus{})
			if zoneErr != nil {
	...
	if err != nil {
		return negannotation.NegStatus{}, err
	}
...
func populateZonesFromCR(serviceName string, svcNegs []negv1beta1.ServiceNetworkEndpointGroup, negStatus negannotation.NegStatus) (negannotation.NegStatus, error) {
	if len(svcNegs) == 0 {
		return negStatus, nil
	}
...
func WaitForStandaloneNegDeletion(ctx context.Context, c cloud.Cloud, s *Sandbox, port string, negStatus negannotation.NegStatus) error {
